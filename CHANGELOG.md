...
- Settings → Appearance rebuilt on the new Compose pattern. Long-press the row to reach the legacy version. App theme picker, app icon (Beta), pure-black dark mode, expanded toolbar, manga-details cover tinting, hide-bottom-nav-on-scroll, and the side navigation controls are all back. The theme picker is the same horizontal tile rail as the legacy widget (mini-preview swatches with toolbar + content + bottom-nav dots in each theme's actual colors). Theme, night mode, pure-black, expanded toolbar, and side-nav-mode changes apply instantly via activity restart, matching the legacy behavior. The follow-system toggle is promoted to a three-way mode selector (Follow system / Light / Dark) so you can force either direction without flipping the device-level preference.
