...
### Additions
- The library has an optional **single-list view** that shows every category as a collapsible section in one scroll, with a floating **category hopper** to jump between categories (previous/next, or pick one from a list) that can be dragged to the start, center, or end of the screen. Turn it on in the library display settings ("Show all categories in one list"); the swipeable category tabs remain the default.
