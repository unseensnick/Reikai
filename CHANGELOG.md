...
### Additions
- Phase 7's **Hide dropped** filter now works for everyone, not only users who hit "Refresh now" on the tracker library. Added `onHoldStatus()` and `droppedStatus()` accessors to the tracker service interface, with overrides in the six full-feature trackers (AniList, MyAnimeList, MangaUpdates, Kitsu, Shikimori, Bangumi). Self-hosted Komga / Kavita / Suwayomi don't model these statuses and continue to be unaffected by the filter
- Personalization sliders and status-based hide filters for the related-mangas carousel. The "Reranking" section under Settings → Library → Recommendations now has two sliders: **Recommendation style** (Popular ← → Personalized, default *Mostly popular*) tunes how heavily the ranker weighs your taste profile against the source's own popularity ordering; **Serendipity** (Familiar ← → Adventurous, default *Mostly familiar*) controls how many exploration slots the ranker reserves for non-personalized picks plus how aggressive the rare-tag novelty boost is. Both sliders are disabled when "Rerank by taste" is off. A new **Filters** section adds two status-based hide toggles — **Hide already-tracked** (Reading / Completed, default on) and **Hide dropped** (default on) — replacing the Phase 6 blanket library-URL hide with a status-aware filter. Library entries without tracker info are still hidden, and Plan-to-read / On-hold entries are now allowed back into the carousel as reminders
- Full-screen "See all" browse view for related mangas. A new "See all (N)" card appears at the end of the carousel when the underlying pool has more than 30 candidates (Madara-style sources + tracker fan-out typically produce 100–200). Tapping opens a dedicated grid showing the full ranked pool with no cap. Long-press a card to enter selection mode with **Add to library** (multi-manga category picker — applies one set of categories to all selected), **Select all**, and **Invert selection**. Tracker-origin cards in a bulk selection are skipped with a toast count, since their URLs don't resolve to installed extensions. The grid scales its column count to the screen width, so foldables and tablets get more columns instead of three oversized ones
- Taste-driven reranking on the related-mangas carousel. New "Reranking" section under Settings → Library → Recommendations with one toggle (**Rerank by taste**, default on). When on, the carousel's source-origin slice reorders against your taste profile: `final = 0.7 × popularity_rank + 0.3 × (taste_score + novelty_boost)`. A handful of slots (~20% of the source slice) stay in the source's original popularity order as exploration picks, and no more than 2 cards in the kept set share the same dominant tag (diversity cap). Tracker-origin entries keep their round-robin fairness ordering regardless. Manga already in your library are also dropped from the carousel — this anti-echo runs whether or not the rerank toggle is on
- Taste profile. New "Taste profile" section under Settings → Library → Recommendations: per-tracker library-pull toggles for AniList / MyAnimeList / Kitsu, auto-refresh interval (Never / 7 days / 30 days), refresh-now button (60 s cooldown), and a last-refresh summary line. Pressing "Refresh now" pulls every entry from your enabled trackers (one GraphQL call for AniList, cursor-paginated REST for MAL, JSON:API with `included` resolution for Kitsu) and caches it locally. Cross-tracker entries (the same manga tracked on AniList + MAL + Kitsu) are deduped at compute time via two cross-ref keys (MyAnimeList id + AniList id), so mirror-list setups don't count the same manga multiple times — AniList carries both ids, MAL only its own, Kitsu resolves both via its `mappings` records
- Active candidate injection on the related-mangas carousel. New "Candidate injection" section under Settings → Library → Recommendations with two toggles, both default on. **Tag search on current source**: runs your top three taste-profile tags as searches against the current source and pools the hits. **Cross-recommendation from favorites**: picks your top-rated tracked manga (normalized score ≥ 0.8, COMPLETED or READING, capped at 5), looks each one up on the current source by title, and pulls the source's native related-mangas list for the match. Both streams feed the same carousel pool as source-native and tracker results, deduped together. Silently produce nothing when the taste profile is empty or no favorite exists on this source

### Changes
- Related-mangas carousel now collapses duplicate manga that appear in more than one stream. If "Solo Leveling" shows up via source-native, AniList recommendations, and your top-rated favorites' related list, you see one card instead of three. Dedup keys on a normalized title (lowercase, trimmed, collapsed whitespace); the first-arriving entry wins, which naturally prefers source-origin entries since they're populated by a single fast call and route directly to the reader
- Related-mangas carousel now activates the source-native related-mangas path for every HTTP-backed extension by default — re-port of Komikku's HttpSource baseline. ~377 Keiyoushi extensions (Madara-derived, GalleryAdults, Iken, plus standalone implementations in koharu / luscious / mangago / mangafire / etc.) immediately surface real related-mangas data from the source you're reading on. Extensions whose details page doesn't carry a parseable manga listing fall through to a popular-parse default that's typically empty — the carousel's existing dedup and 30-cap absorb any noise. An earlier port of this baseline was reverted in the same release cycle; the audit of the Keiyoushi extension repo proved the reversal premature New "Taste profile" section under Settings → Library → Recommendations: per-tracker library-pull toggles for AniList / MyAnimeList / Kitsu, auto-refresh interval (Never / 7 days / 30 days), refresh-now button (60 s cooldown), and a last-refresh summary line. All three trackers have live library fetchers — pressing "Refresh now" pulls every entry from your enabled trackers (one GraphQL call for AniList, cursor-paginated REST for MAL, JSON:API with `included` resolution for Kitsu) and caches it locally for the profile. Cross-tracker entries (the same manga tracked on AniList + MAL + Kitsu) are deduped at compute time via two cross-ref keys (MyAnimeList id and AniList id) — important for mirror-list setups where the same manga would otherwise count multiple times. AniList trivially carries both ids; MAL only its own; Kitsu resolves both via its own `mappings` records. The AniList-id fallback catches manhwa where MAL cross-refs don't exist. No user-visible recommendation changes yet — the data is collected so Phase 5+ (active candidate injection, taste-driven rerank) can consume it
...
- `SetCategoriesSheet` now correctly flips `favorite` + `date_added` for every manga in a multi-manga selection, not just the first. Drops the matching pre-flip workaround in the bulk-add path under Phase 6.5's "See all" browse
