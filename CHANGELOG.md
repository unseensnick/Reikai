...
- **Light novels in your library can check for new chapters in the background.** Reikai can periodically re-check each favorited novel's source and sync any new chapters so your unread counts stay current, optionally **auto-downloading** them as they arrive. Configure it under **Settings → Library → Light novel updates**: choose an interval (off by default, or every 12 hours up to weekly), set device restrictions (only on Wi-Fi, only on an unmetered network, or only while charging), and optionally limit it to **ongoing novels** (skipping completed ones). Turn on auto-download with the **Novels** switch under **Settings → Downloads → Download new chapters**, where novels get the same options as manga: **skip downloading chapters you've already read** and **limit auto-download to chosen categories**. A progress notification with a Cancel action shows while it runs, and a result notification reports how many novels gained new chapters.
