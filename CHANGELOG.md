...
### Additions
- Active candidate injection — tag search + cross-recommendation (Phase 5 of the personalized-recommendations feature). New "Candidate injection" section under Settings → Library → Recommendations with two toggles, both default on. **Tag search on current source**: runs the user's top three taste-profile tags as searches against the current source and pools the hits. **Cross-recommendation from favorites**: picks the user's top-rated tracked manga (normalized score ≥ 0.8, COMPLETED or READING, capped at 5), looks each one up on the current source by title, and pulls the source's native related-mangas list for the match. Both streams flow into the existing carousel pool with source-origin candidate type, so they dedupe and slot-share with source-native and tracker streams. Silently produce nothing when the taste profile is empty, no favorite exists on this source, or the source's search/details doesn't match the expected shape
- Taste profile groundwork (Phase 4 of the personalized-recommendations feature).
