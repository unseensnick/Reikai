...
- Compose library, novel surfaces, and the in-app WebView now share a single small top bar component, so titles, navigation icons, and action clusters look and behave consistently across screens. The large collapsing toolbar style is retired from the Compose surfaces; toggling **Settings → Appearance → Use large toolbar** still affects the legacy chrome but no longer changes the Compose library, the Settings / About screens, or the LN screens, which all render the small toolbar.
