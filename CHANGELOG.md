...
### Additions
- **Support for TachiyomiX 1.6 extensions.** The newer extension format now installs and loads (your existing extensions keep working), letting sources attach hidden metadata to manga and chapters that is carried through backups. Older backups made before this still restore cleanly.
- **Light-novel library grouping.** The novel library can now group by source, tag, author, language, or status, or stay ungrouped, instead of by your categories, chosen from a new **Group** tab in the library settings (matching the manga library). Works in both the tabbed and single-list views, and groups collapse like categories do, including sinking a collapsed group to the bottom when the "move collapsed groups to the bottom" display option is on (and the hopper's collapse-all).
...
- Two new reader options under **Settings → Reader → Reading**: **"Resume reading position"** (off by default) reopens chapters at your last position even if they're already marked read, handy for long webtoons; and **"Pages to preload"** lets you load more pages ahead so flipping forward waits less (default 4).
...
- The reader and manga details screen can now be **tinted with each manga's cover color** (Settings → Appearance → "Theme based on cover", on by default), with a choice of color styles. Turn it off to keep the normal app theme. The cover-derived reader background ("Automatic" in the reader's background options) is independent, so you can use either or both.
- The reader's **bottom bar buttons are now customizable** (Settings → Reader → Reading → "Bottom bar buttons"): choose which of view-chapters, WebView, browser, share, reading mode, rotation, and crop borders appear. A new **chapters button** opens an in-reader list of all chapters where you can jump to any chapter, bookmark it, or download it without leaving the reader.
...
- **Light novels now live in your Library** behind a **Manga / Novels** chip below the toolbar (the library shows one type at a time; both the swipeable tabs and single-list views, the cover badges, and category grouping all work as they do for manga). **Long-press a novel to multi-select** (drag or long-press a second to range-select), then **download** (a menu for the next 1/5/10/25, all unread, or bookmarked chapters), **mark read or unread**, **change category**, or **remove from library** (optionally deleting its downloads). The filter icon opens a **Filter / Sort / Display** sheet: **filter** by downloaded, unread, started, completed, or bookmarked; **sort** each category on its own by title, last read, last updated, latest chapter, chapter fetch date, total chapters, unread count, date added, downloaded, or random (tap the active mode to flip direction); and the **Display** tab shares the manga library's display settings, plus an optional **Continue reading** button on covers that jumps to the next unread chapter. **Novel categories are created and managed from the same Edit categories screen as manga**, behind the same Manga / Novels chip.
- **Browse now spans manga and light novels with an All / Manga / Novels filter** on both the Sources and Extensions tabs (your choice sticks, and the two tabs stay in sync). Add light novel sources by opening Browse → Extensions, switching to Novels, and using the overflow's Repos screen to add an LNReader plugin repository (a combined manga + light-novel repo manager); installed novel sources then appear under Browse → Sources alongside your manga sources. Plugin updates are checked in the background and surfaced as a count on the Extensions tab.
...
- **Browsing a light novel source now shows a cover grid** styled to match the manga catalogue, opened by tapping a source under Browse → Sources (Novels). **Popular** and **Latest** chips switch the listing, a **Filter** sheet renders the source's own options (sort, genres, status, and so on), and sources that need configuration (a login, a server URL) get a **Settings** sheet from the toolbar menu. Search runs from the toolbar, results **load more as you scroll** (stopping at the end of the source), and novels **already in your library are marked with an "In library" badge**. **Long-press a result to add it to your library**: if a similarly-named novel is already saved you get a **possible-duplicates prompt** (open one of them, or Add anyway), and adding lets you **pick categories** (none chosen drops it in the default category); long-press an in-library result to **remove** it. A toolbar **display-mode switch** offers comfortable grid, compact grid, or list. A failed or Cloudflare-blocked load offers **Retry** and **Open in WebView**, and **returning from that WebView re-runs the load automatically** (so solving a Cloudflare check then backing out shows the results without tapping Retry).
- **Light novel global search.** On Browse → Sources with the Novels filter active, the search action looks your query up **across all your installed light novel sources at once**. Each source shows its own row of results that fills in as it finishes, with its own loading, "no results", or error.
...
- **Reading a light novel now opens a full-screen reader.** Chapters render with their formatting (paragraphs, emphasis, images) and LNReader-style typography, with a Display / Theme settings sheet for **font size, line height, margins, a choice of bundled fonts, and text alignment**, plus **light / sepia / mint / dark / black themes or follow-system**, all applied live. **Tap to toggle an immersive view** that hides the system bars, your **scroll position is saved and restored** per chapter, and you can move to the **next or previous chapter** without leaving the reader, with the next chapter prefetched so flipping forward is instant. Reading to the end marks the chapter read. A new reader setting, **Settings → Reader → Reading → "Skip duplicate chapters"** (off by default, sitting beside the manga one with each labelled by type), makes next/previous skip over a chapter whose number repeats the one you just read, handy for a merged novel where the same chapter comes from more than one source.
- **Light novel chapters can now be downloaded for offline reading.** Tap the download icon on a chapter in a novel's details to save its text, with any inline images embedded into the saved chapter so they show offline too; a downloaded chapter opens instantly with no network and works in airplane mode. The download icon opens a menu to **start it now, cancel a queued one, or delete a finished download**, and you can **long-press to select chapters then tap Download** (or Delete) to batch them. Downloads run in the background on a single queue with a progress notification (with a Cancel action), keep going if you leave the screen or close the app, and resume after a restart. The queue is **paced per source and automatically eases off if a source is slow or starts refusing**, so a large batch stays gentle on the site. The **download queue** (More → Download queue) gained an **All / Manga / Novels** filter so novel downloads sit alongside manga ones, and **Settings → Downloads → Delete chapters** gained a light-novel twin of the "After manually marked as read" option that deletes a chapter's offline copy once you finish it.
- **Light novels in your library can check for new chapters in the background.** Reikai can periodically re-check each favorited novel's source and sync any new chapters so your unread counts stay current, optionally **auto-downloading** them as they arrive. Configure it under **Settings → Library → Light novel updates**: choose an interval (off by default, or every 12 hours up to weekly), set device restrictions (only on Wi-Fi, only on an unmetered network, or only while charging), restrict it to chosen **categories** (include/exclude), and use **Smart update** to skip novels that are completed, fully read, or unstarted. Turn on auto-download with the **Novels** switch under **Settings → Downloads → Download new chapters**, where novels get the same options as manga: **skip downloading chapters you've already read** and **limit auto-download to chosen categories**. A progress notification with a Cancel action shows while it runs, and a result notification reports how many novels gained new chapters.
- **Light novels now appear in the Updates tab.** An **All / Manga / Novels** chip below the Updates title brings recent new chapters of your favorited novels alongside manga in one unified screen; the **All** view interleaves both by date. The **filter** (downloaded / unread / started / bookmarked, plus a new **by-category** include/exclude filter) applies to manga and novels together; because manga and novels keep separate categories, the category picker shows a section per type (and just that type's section on the Manga or Novels chip). The upcoming-releases **calendar** stays with manga, and you can **long-press to select** entries (across both types) to mark read, bookmark, download, or delete. A **Group by series** option in the filter (off by default) collapses a series' multiple same-day new chapters into one expandable "N new chapters" row, for manga and novels alike; a series you've merged across sources collapses as one. Tap a novel update to start reading it, or download it from the row. With novels in the feed, the **"Show unread count on Updates icon"** badge now counts new novel chapters too.
- **Light novels from the same series across multiple sources now merge into one** (the novel counterpart of the manga merge). **Long-press to select novels in the library and tap Merge** to group them under a single cover (works in both the tabbed and single-list views), or **Unmerge** to split a merged selection back apart. Novels with the **same title** are grouped automatically, with a safety guard you control in the library's **Display** settings: **"Also require matching author"** (on by default) only auto-merges same-title novels that also share an author, so two unrelated novels with a common title stay separate; turn it off for title-only grouping. A **"Show source icons on merged covers"** display option puts each grouped source's icon on the merged cover (up to three plus a +N) instead of a plain count. **Settings → Advanced** gains actions to clear manual novel merges or separate every merged novel.
- Several light-novel library fixes: **novel covers that previously failed to load** in the library, on details, and in browse now load (novels now have their own cover loader that sends each source's real-browser headers, the user agent and referer some sites require); **novel covers now show the source icon badge** (toggle in the library display settings) like manga; the **language badge shows the 2-letter code** (EN) like manga instead of the full language name; a **category's sort label now reflects the sort you picked** (it could show a stale label before); the **Display → Category sort order setting (A→Z / Z→A) now reorders novel categories too** (it previously only affected manga); and the **category hopper's long-press actions are now novel-aware** (random, collapse-all, and the settings shortcut act on novels and open the novel settings, not manga). **Settings → Browse → Extension stores** now shows both the manga and light-novel repository counts.
...
- **Light novels with paged chapter lists now load a page at a time.** Sources that split a novel's chapters across multiple pages (some span thousands of chapters) show a **page selector** under the chapter count: pick a page to load just that page's chapters, so opening a huge novel stays fast. Sort and filter apply to the page you're viewing, mark-all and mark-previous span every page you've loaded, and refreshing re-checks the page you're on plus any newly added pages.
- The Compose novel details screen reached closer parity with the manga screen: the **Continue/Start reading button now names the chapter** and **collapses as you scroll**; chapter rows show their **upload date (as relative dates)** and **reading progress (% read)**, and you can **swipe a chapter to mark it read or bookmark it** (per your manga swipe-action settings). **Tap the title, author, genres, or source to search them; long-press to copy.** **Tap the cover** to zoom, save, share, or **set a custom cover from your gallery** (cached, and kept until you delete it). **Edit info** now also covers a **status override** and a **Reset to source** button. For a novel merged from several sources, switching to a specific source chip shows that source's own description, status, and cover coherently (editing while on that chip edits that source), and the **combined chapter list labels each chapter with the source it came from**. The chapter selection bar's **Hide / Unhide** action is now an icon.
...
### Fixes
- The library's **Continue reading button now also appears in the single-list view**; it was previously only on the swipeable category pager.
