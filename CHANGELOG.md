...
### Additions
- **Manga from the same series across multiple sources now share a combined chapter list.** When the same title is in your library from several sources, opening its details shows one merged, de-duplicated chapter list pooled from every source (matched by chapter number); each chapter opens and downloads from its own source, and marking read or bookmarked applies across the group. Use **Manage sources** in the overflow menu to split a source back out or remove it from your library, both with Undo. A tracker check automatically separates same-titled entries that are actually different series.
