...
### Additions
- The library **Display options** sheet was reworked into a Komikku-style tabbed panel (Filter / Display / Group / Badges / Categories): it **floats as a centered dialog on tablets and foldables** and stays a bottom sheet on phones, tabs are **swipeable** and scroll horizontally, filter options use **segmented controls** instead of chips, and the grid-size slider is now full-width with a live "Portrait / Landscape" size readout. A new **Group** tab holds the group-by setting (moved out of Categories).
- When **"show all categories" is off**, the library shows a **scrollable category tab row** at the top to switch categories, in addition to the hopper and swipe.
