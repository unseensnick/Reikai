...
### Additions
- **Light novels are now a first-class part of your library.** A row of Manga / Novels filter chips below the toolbar switches the library between the two; novels get the same covers grid, category grouping (in both the swipeable-tabs and single-list views), cover badges, multi-select (download next 1 / 5 / 10 / 25, all unread, or bookmarked; mark read or unread; change category; remove), and a Filter / Sort / Display sheet. Novel categories are created and managed from the same Edit categories screen as manga. *(Under the hood: a net-new NovelLibraryScreenModel with novel-specific preference keys, reusing the library UI.)*
- **Browse spans manga and light novels.** An All / Manga / Novels filter on the Sources and Extensions tabs (kept in sync) lets you add LNReader plugin repositories from the Repos screen, install novel sources, and browse them in a cover grid styled like the manga catalogue: Popular / Latest chips, a source filter sheet, a source settings sheet (login, base URL), search, load-more, in-library badges, and long-press to add with a possible-duplicates prompt and category pick. *(Under the hood: NovelSourceManager loads plugins once per app launch; NovelBrowseScreen mirrors the manga catalogue.)*
- **Light novel global search** looks your query up across every installed novel source at once, each source filling in its own row of results with its own loading, empty, and error states.
- **A full novel details screen** matches the manga layout: a blurred cover backdrop, an expandable description with genre chips, the chapter list, and an Add to library toggle with category assignment. It supports chapter multi-select (with two-finger range selection), a Filter / Sort / Display sheet, hideable chapters, Edit info (title, author, artist, description, genres, and status, with a reset-to-source), WebView and Share, a Resume / Start reading button, and per-page loading for novels whose chapter lists span thousands of entries. Saved novels open from local storage (instant and offline) and re-check the source only on a refresh. *(Under the hood: net-new NovelDetailsScreenModel, NovelChapterFlags, NovelPageSelectorSheet.)*
- **A full-screen light novel reader.** Chapters render with their formatting (paragraphs, emphasis, images) and LNReader-style typography. A Display / Theme sheet controls font size, line height, margins, bundled fonts, text alignment, and light / sepia / mint / dark / black themes (or follow-system), all applied live. Tap toggles an immersive view, your scroll position is saved per chapter, and next / previous walks chapters with the next one prefetched. A "Skip duplicate chapters" setting skips a repeated chapter number across merged sources. *(Under the hood: a net-new NovelReaderWebView over bundled lnreader-web assets.)*
- **Offline downloads for light novels.** Save a chapter's text (with inline images embedded) from its details, one at a time or in batches; a single background queue, paced per source and easing off when a source is slow, shows a progress notification and resumes after a restart. The Download queue gains an All / Manga / Novels filter, and Settings → Downloads can delete a novel chapter's copy once you finish it. *(Under the hood: a net-new text-only NovelDownloadManager.)*
- **Light novels update in the background and show in the Updates tab.** Reikai periodically re-checks favorited novels for new chapters (configurable interval, device restrictions, category include / exclude, and a Smart update that skips completed or fully-read novels), optionally auto-downloading them. New novel chapters appear in a unified Updates feed behind an All / Manga / Novels chip, sharing the filters, group-by-series, long-press actions, and the unread-count badge. *(Under the hood: a net-new NovelUpdateJob on WorkManager feeding a shared Updates screen.)*
- **Light novels from the same series across sources merge into one.** Long-press to Merge or Unmerge, or let same-titled novels group automatically (guarded by an optional "also require matching author"). A merged novel shows one cover (with the grouped sources' icons), one deduplicated chapter list with a source switcher, and read / bookmark state shared across sources. A Preferred sources screen (split into Manga and Light novels tabs) ranks which source leads a merged list. *(Under the hood: a net-new NovelMergeManager with preference-based merge groups.)*
- **Light novel plugins stay current.** The Browse badge counts pending plugin updates alongside manga extensions, with a background check every 12 hours (plus a cache-gated check on launch) and a notification on its own channel; outdated rows get an Update button. Plugins show their real icon from the LNReader registry instead of a generic book glyph. *(Under the hood: a net-new LnPluginUpdateJob.)*
- **Cross-source merge for manga.** When the same series is in your library from several sources, it shows as a single cover with combined unread counts, and on the details screen as one deduplicated chapter list pooled across sources behind a source switcher. Merge or unmerge by hand from library multi-select, or let same-titled series auto-merge; Manage sources splits a source back out or removes it (with Undo), and a Preferred sources ranking decides which source's chapters lead the merged list. *(Under the hood: a net-new preference-based merge engine, MangaMergeManager.)*
- **Related-manga recommendations carousel on manga details.** Below the description, a Related row suggests similar titles (with in-library badges) from the source and, when enabled, from AniList, MyAnimeList, MangaUpdates, and Shikimori (even for manga you don't track). Tap a source suggestion to open it, or a tracker suggestion to look it up via global search. A See all grid lets you multi-select and add several at once (with category handling), group by where each came from, and reveal hidden ones. A new Settings → Library → Recommendations screen turns tracker recommendations on or off per tracker, opts each connected tracker's library (AniList, MyAnimeList, Kitsu, Shikimori, Bangumi) into a taste profile that reorders the row, adds "Because you're reading..." and "Matching your taste" picks for tracked titles, and offers Recommendation style and Serendipity controls, an optional weekly or monthly auto-refresh, and filters that hide titles already in your library or tracked with a chosen status (all filters off by default). Results cache for 30 minutes, near-duplicate titles collapse into one, and titles several sources agree on rank higher. *(Under the hood: the source-native related-manga contract and the AniList / MyAnimeList tracker-recs approach are inspired by Komikku / TachiyomiSY; the taste-profile reranker, the MangaUpdates provider, caching, and the unified carousel (RecommendationRanker, RelatedMangaCache) are Reikai's own, carried over from the Yokai base; the Shikimori provider is new in this rebase.)*
- **Two-finger range selection on chapter lists.** On a manga or novel details screen, press two chapter rows at once to select every chapter between (and including) them, a quick alternative to long-pressing the start then the end.
- **Dynamic library grouping.** Group the library by source, tag, author, language, status, or tracking status instead of by your categories, chosen from a Group tab in the display settings; each group collapses like a category, and collapsed groups can sink to the bottom. Works in both library views, for manga and novels. *(Under the hood: a net-new LibraryDynamicGrouping kernel; the Group tab is added to the display sheet.)*
- **Single-list library view with a category hopper.** An optional view shows every category as a collapsible section in one scroll, with a floating hopper to jump between categories (draggable to the start, center, or end; a configurable long-press action; hideable). Each category header has its own sort (tap to change it or flip the direction), a refresh button, and a select-all. The swipeable category tabs remain the default. *(Under the hood: the single-list view and hopper are carried over from the Yokai base (Reikai's previous version), re-implemented in Compose as ReikaiLibraryContent and ReikaiCategoryHopper; per-category sort reuses Mihon's SetSortModeForCategory.)*
- **Category sort order and hidden categories.** A Category sort order setting (Off / A to Z / Z to A) orders the categories everywhere they are listed: the library, the hopper's jump list, the category filter, and the category manager. Categories can be hidden without deleting them (with a Show hidden categories toggle to bring them back), and the hidden state round-trips through backups, including with Komikku. *(Under the hood: the hidden flag rides the existing category flags, so no migration.)*
- **Library update-errors screen.** Turn on Settings → Advanced → Track library update errors and an Update errors entry in the library overflow lists the entries that failed their last update, grouped by reason, to review, dismiss, or retry. Off by default. *(Under the hood: a net-new UpdateErrorsScreen and domain layer.)*
- **Panorama grid mode and source-icon cover badges.** A comfortable-grid mode shows wide cover art uncropped (portrait covers unchanged), and covers can show the source's icon. *(Under the hood: both ported from Komikku, re-typed onto Reikai's models.)*
- **Lewd-content and category filters in the library filter sheet.** Filter the library by an adult-content heuristic, or include / exclude specific categories. *(Under the hood: both ported from Komikku.)*
- **New reader options:** Resume reading position (reopen a chapter at your last spot even if already read), Pages to preload (default 4), and Mark chapter read when skipping ahead (forward skips only, respects incognito). *(Under the hood: net-new reader preferences; mark-read-on-skip is fenced in a `// RK` island.)*
- **Customizable reader bottom-bar buttons and an in-reader chapters list.** Choose which buttons appear on the reader's bottom bar (view chapters, WebView, browser, share, reading mode, rotation, crop borders); a chapters button opens an in-reader list to jump to, bookmark, or download a chapter without leaving the reader. *(Under the hood: both ported from Komikku, re-typed onto the reader.)*
- **Cover-color theming.** The reader and manga details can tint with each manga's cover color (Settings → Appearance → "Theme based on cover", on by default), with a choice of color styles. The cover-derived reader background ("Automatic") is independent, so you can use either or both. *(Under the hood: carried over from the Yokai base; the getBestColor palette heuristic originates with Jays2Kings (J2K).)*
- **FlareSolverr support for bypassing Cloudflare.** When a source is blocked by a Cloudflare challenge, route it through a [FlareSolverr](https://github.com/FlareSolverr/FlareSolverr) proxy instead of the in-app WebView, which handles stricter protection and outdated device WebViews. Turn it on in Settings → Advanced → Networking: enable FlareSolverr, enter the server URL, and use the Test button to confirm it works and sync the user agent. The WebView solver stays the default and the fallback when FlareSolverr is off. *(Under the hood: a net-new FlareSolverrClient, handling GET and POST.)*

### Changes
- **Renamed the fork to Reikai.** The Yokai-Y2K placeholder name is retired: installs upgrade in place (same package ID), and the launcher shows the new R-monogram icon and "Reikai" label. *(The `.y2k` package suffix stays under the hood.)*
- **The library Display options sheet is now tab-aware.** Filter chips, the Categories tab, and the Edit categories button each act on whichever library (manga or novels) is active, so a filter applied on the Novels tab no longer reaches into the manga library. *(Manga and novels each get their own settings dialog and preference keys.)*
...
### Fixes
- **The light novel reader no longer crashes on a chapter with repeated paragraphs** (blank lines, scene breaks, recurring phrases). *(Paragraphs are keyed by position instead of by text, so identical ones no longer collide.)*
- **Adding a light novel no longer creates a duplicate library entry.** Adding the same novel again (for example after browsing for it) now resolves to the existing entry and just flags it as a favorite.

### Other
- **Reikai is now built on the Mihon base.** The previous release was a fork of Yokai; this cycle rebases the app onto Mihon, so the core manga reader (library, manga details, reader, tracking, extensions, backups) is Mihon's, with Reikai's own features (light novels, cross-source merge, recommendations, and the library, reader, and theming additions above) rebuilt on top. This is why the core UI looks different; the `.y2k` package id is preserved so existing installs upgrade in place.
- **Support for TachiyomiX 1.6 extensions,** brought in via the Mihon sync: the newer extension format installs and loads (existing extensions keep working), and sources can attach hidden metadata to manga and chapters that is carried through backups; older backups still restore. *(Under the hood: Mihon's ExtensionLoader 1.6, ported during an upstream sync.)*
- **Synced upstream changes from Mihon:** library updates (Coil, OkHttp, Firebase), a SQLite driver build that avoids a rare database stall when a write is cancelled, tying background tasks (downloads, library updates, source loading) to the app's lifecycle so they no longer outlive it, and auto-following extension repositories that moved to the newer index format.
