...
### Additions
- **Private tracking** for AniList and Kitsu: bind a title privately from the tracker search screen (the eye-off button next to "Track"), or toggle "Track privately / Make public" from a tracked entry's overflow menu. Private entries stay hidden on your public AniList/Kitsu profile and show an eye-off badge in the tracking sheet. (Bangumi will follow once its tracker API is upgraded.)
...
- The Compose library's manga details screen gains a **chapter filter / sort** sheet, opened from the new filter icon in the details toolbar. Filter by unread, downloaded, or bookmarked (each cycles show-all / show-only / hide), sort by source order, chapter number, or upload date (tap the active option to flip direction), and hide chapter titles to show "Chapter N" instead of the source name. On series with multiple scanlator groups, **Filter groups** lets you hide specific groups. "Set as default" / "Reset" carry your filter and sort choices across to every series.
- The Compose library's manga details screen can now **download chapters**. Each chapter row has a download button showing live state (queued, downloading with progress, downloaded, or error); tap to download, tap a downloaded chapter to delete it, or tap a downloading one to cancel. The toolbar overflow adds **Download next / next 5 / unread / all**.
- The Compose library's manga details screen now supports **chapter multi-select**. Long-press a chapter to start selecting, tap others to add or remove, and long-press a second chapter to select the whole range in between. The contextual bar offers Mark as read / unread, Bookmark / Remove bookmark, Download, Delete, Mark previous as read / unread, Select all, and Invert selection; system back clears the selection.
...
- The library Display options sheet is now **tab-aware**. Filter chips, the Categories tab toggles, and the Edit/Add categories button each operate on whichever library is active (manga or novels), so collapsing categories or applying a filter on the Novels tab no longer reaches into the manga library. A new **Settings → Advanced → Share library display settings between manga and novels** toggle (on by default, status-quo behaviour) controls whether the visual settings — grid size, badges, layout — stay in sync across both libraries or each tab keeps its own. Manga-only toggles (the category hopper trio, *Always show current category*, *Series type* / *Content type* filter chips) are hidden on the Novels tab in both modes. The novel library's render path now reads each visual preference from its own store when independent mode is selected. A one-time migration seeds the novel keys from the existing manga values on first launch so flipping to independent mode starts from the user's existing look.
- **Compose library top bar matches the legacy library shape.** The Compose library (and the Light novels tab hosted inside the legacy library) now renders a large headline "Library" title with a tappable search card directly below it ("Search '\<random library entry\>'" when search suggestions are on, "Search title, tags, source" otherwise), the Manga / Light novels tab row immediately under the card, and Filter + Overflow icons in the trailing corner. Scrolling collapses the headline while keeping the search card and tabs pinned at the top. Tap the search card to open the keyboard-driven search; tap the filter funnel for the existing Display options sheet. The filter / overflow icons now use the Material Tune and More-vert glyphs (matching the Settings, Browse, and other Compose surfaces) instead of the older legacy drawables.
