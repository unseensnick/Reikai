...
- Saved novel details now use the same layout as the manga details screen: a blurred cover backdrop behind the cover, title, author, status and source, an expandable description with genre chips, and the chapter list, plus an **Add to library** toggle with category assignment. Backing out of the in-screen reader returns to the novel's details instead of the library, and the description, cover, author and status now refresh from the source on pull-to-refresh.
