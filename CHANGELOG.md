...
### Additions
- **Related-mangas carousel on manga details.** Below the description (and in the left pane on tablet), a horizontal **Related** row suggests similar titles from the source, ranked for relevance, with an **in-library badge** on ones you already have; tap a cover to open it. Suggestions that differ only by accents, fullwidth characters, or punctuation are collapsed instead of showing twice. (Tracker-based recommendations and a personalized taste profile are being added on top of this.)
