...
- **Browsing a light novel source now shows a cover grid** styled to match the manga catalogue, opened by tapping a source under Browse → Sources (Novels). **Popular** and **Latest** chips switch the listing, a **Filter** sheet renders the source's own options (sort, genres, status, and so on), and sources that need configuration (a login, a server URL) get a **Settings** sheet from the toolbar menu. Search runs from the toolbar, results **load more as you scroll** (stopping at the end of the source), and novels **already in your library are marked with an "In library" badge**. A failed or Cloudflare-blocked load offers **Retry** and **Open in WebView**, and **returning from that WebView re-runs the load automatically** (so solving a Cloudflare check then backing out shows the results without tapping Retry).
