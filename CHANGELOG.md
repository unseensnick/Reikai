...
- **The novel download queue now shows one card per series that you can reorder or cancel.** A full-novel download is a single card with its progress instead of thousands of chapter rows; drag it, bump it to the top or bottom, or cancel the whole series, and it pauses instead of erroring when the connection drops.
