...
- Several light-novel library fixes: **novel covers that previously failed to load** in the library and on details now load (some sources only serve covers to a real browser, which the cover loader now sends); the **language badge shows the 2-letter code** (EN) like manga instead of the full language name; a **category's sort label now reflects the sort you picked** (it could show a stale label before); and the **category hopper's long-press actions are now novel-aware** (random, collapse-all, and the settings shortcut act on novels and open the novel settings, not manga). **Settings → Browse → Extension stores** now shows both the manga and light-novel repository counts.
