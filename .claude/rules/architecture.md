...
- **A dependency whose construction must stay deferred** is a `() -> T` parameter, never a lazy delegate: the graph builds it when the parameter is called rather than when the class is. `NovelDownloadManager` is the one that matters, because building it restores the persisted download queue and can start the worker. Metro 1.4.3 deprecated the `Provider<T>` spelling this used to name; the two compile identically, and the tree moved to `() -> T` wholesale rather than carry 46 warnings.
