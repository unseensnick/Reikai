...
**Injekt is not gone, and the two survivors are permanent or ruled.** `source-api` and `source-local` keep it because they are the contract installed extensions compile against, and `eu/kanade/domain/DomainModule.kt` keeps three registrations, `source-api`'s `MetadataSource` contracts. `MetroInteropModule` hands graph-owned singletons back to Injekt so extensions can still resolve them. Do not add to any of that: net-new code goes in the graph. `DomainModuleTest` resolves the three for real, so removing one that is still needed fails there rather than at runtime.
