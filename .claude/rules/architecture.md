...
- **Edits to Mihon's own files** (the nav tab list, backup proto fields, DI registration) are fenced with `// RK -->` / `// RK <--` comment islands so they survive upstream merges and are greppable. Mirrors Komikku's `// SY` / `// KMK` convention. Every changed hunk sits under a marker (an island, a one-line `// RK:` note above it, or a trailing `// RK`); imports are exempt. A member deleted from a Mihon file leaves a one-line `// RK:` note where it stood, naming its replacement (`SourcePreferences.lastUsedSource`), since a sync otherwise cannot tell a deliberate removal from a lost one; that is a note, not the kept remnant content-layer.md forbids.
