...
- No plan/roadmap codename markers in code comments (`Phase N`, `Stage N`, the `P5 S5` phase/slice shorthand, `Y3` Yōkai-era feature refs, `R3` roadmap refs, `Active #N`, or a plan-style `Step 3` reference). They rot as the plan moves on and become noise that has to be stripped later. State the durable fact instead. A colon-led algorithm step (`Step 1:`) is fine. Enforced by the `pre-commit` hook on added lines and by `docs-lint` CI across the whole tree, both from `scripts/lint-docs.sh` so neither can get stricter than the other; both spare `R8` (the code shrinker) and `M3` (Material 3). The step match is case-insensitive: a lower-case `step 2` reads exactly as a plan reference and used to slip through.
