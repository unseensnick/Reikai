...
- **No standalone refactor sprints**: Refactor incrementally alongside the feature or fix that motivated it. Never propose a separate "cleanup pass" unless the user asks. **Two standing exemptions**, both owner-approved refactor initiatives, so work under either needs no motivating feature: the content-layer program (the manga/novel collapse), and the settings rewrite scoped in [settings-restructure.md](../../docs/dev/plans/settings-restructure.md) (owner, 2026-09-03), which is bounded to the screens that doc names. Nothing else is exempt, and neither exemption licenses cleanup on an adjacent file just because it was open.
...
- No plan/roadmap codename markers in code comments (`Phase N`, `Stage N`, the `P5 S5` phase/slice shorthand, `Y3` / `Y11` Yōkai-era feature refs, `R3` / `R12` roadmap refs, the `R-feature` / `Y-feature` tags, `Active #N`, or a plan-style `Step 3` reference). They rot as the plan moves on and become noise that has to be stripped later. State the durable fact instead. A colon-led algorithm step (`Step 1:`) is fine. Enforced by the `pre-commit` hook on added lines and by `docs-lint` CI across the whole tree, both from `scripts/lint-docs.sh` so neither can get stricter than the other; both spare `R8` (the code shrinker), `M3` (Material 3) and `Y2K`. The step match is case-insensitive: a lower-case `step 2` reads exactly as a plan reference and used to slip through.
