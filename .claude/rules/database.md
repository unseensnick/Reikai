...
- Backup-restore compatibility: Reikai uses Mihon's backup format (`.tachibk`), and backup files don't bind to `applicationId`. Backups do interchange with pre-rebase Yōkai-based builds, because the proto is shared and unknown fields are skipped: a Yōkai-era `.tachibk` restores its library, categories, chapters, history, tracking, settings and custom info. Two known limits, both in `BackupCategory`: Yōkai keeps per-category sort in field 800, which Reikai does not read, so it does not carry over; and Yōkai writes no field 3, so every category decodes with `id = 0` and the category-id settings `PreferenceRestorer` remaps by that id (default category, update include/exclude) do not restore correctly. Keep the format compatible with Mihon's so a backup round-trips across Reikai versions.
