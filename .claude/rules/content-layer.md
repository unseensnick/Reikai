...
| Downloads | Takeover of orchestration, complete | One queue engine over two adapters owns the list, its saved order, the cards, the per-series chapter sheet and the queue actions; `DownloadManager` and `NovelDownloadManager` stay separate engines running side by side. Shared rules sit in kernels both call (`SeriesCompletions`, `DownloadIndexRules`, `downloadedOrCurrent`). Pacing is novels only, since manga extensions rate-limit their own clients. Mihon's queue model and its RecyclerView are deleted and manifested | content-layer-download-surface |
| Reader | Takeover of orchestration, complete (what each step landed lives in the record's Status, not here) | One host Activity keeps the window responsibilities, a shared engine owns navigation, position, the chrome composables, the action verbs and **the viewer contract** over two providers. Menu visibility stays with the host, because half of it is the insets controller. `ReaderViewModel` stays **live** as the manga provider; the novel reader dissolves entirely, since its engine is being replaced by two rendering modes, a native renderer ported from tsundoku and a WebView mode rendering Reikai's own document. Mihon's `Viewer` becomes one adapter under the engine's contract rather than the interface novels implement, because `ViewerChapters` cannot carry a novel chapter without the novels-as-manga mechanism the program rejected. **The image viewers are never diverged past the divergences they already carry**, which are named because the count is not greppable: 49 `// RK` lines under `ui/reader/viewer/` express four pre-takeover mechanisms (the cover seed-colour theming, the per-source next-chapter-downloaded badge, that badge's off-main-thread resolution, and the user-settable volume-key scroll distance) plus five islands the owner ruled in: `WebGpuViewer.setChapters`, the chapter jump; `WebGpuConfig`, which reads the crop-borders setting of the mode its viewer is serving, since one viewer class serves both and the paged key left the control acting on nothing in long strip; and `WebGpuViewer`'s failed-load retry, which draws a failed page or neighbour chapter with a Retry that a tap in the middle of the screen runs, in place of upstream's endless progress ring and 5-second refetch, with its rules in `WebGpuRetryRules.kt`; and `MissingChapters.calculateChapterGap`, which counts a boundary's missing chapters by the chapter list's owner-aware rule (`ChapterGap.atSeam`), since two sources of a merged series number apart; and `WebGpuViewer.getPage`'s page identity, which drops the cached wrapper of a page its chapter replaced by loading again (a reload, or a chapter released and opened again, which upstream also leaves stale), with its rule in `WebGpuPageCache.kt`. Position is a typed capability, never an `Int`, with `ReaderChapter.requestedPage` as the one named exception, since the three viewers read it directly. **The takeover owns the settings whose implementation lives in the code being deleted**, which on this surface is most of the novel reader's typography | content-layer-reader-surface |
| Sources | One seam over three source kinds, in progress | Every novel feature reaches a source only through `NovelSource`, over three kinds: LN plugins, tachiyomi-format APKs and IReader APKs, each through one adapter. Kind differences (filters, settings, search filters, image requests, listings, commands) are typed capabilities. The APKs arrive through Mihon's one extension pipeline, which learns the two extra manifest features as `// RK` islands, never a parallel loader. Source-side tracking dispatch and tracker auto-binding are shared kernels over both content types | content-layer-sources-surface |
...
- **Dead code is deleted, never marked** (owner, 2026-08-12), in a live Mihon file as much as in a
  ...
  a sync needs is `refs/mihon`, not a corpse in this tree.
