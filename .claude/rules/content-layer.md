...
| Browse | Behavior-partial | Shared bulk-favorite generic, shared dialogs and the default-category kernel. Verbs stay per-type and the pager and filter dispatch stay declined on mechanism grounds. The add decision, the write order and the duplicate dialog are now shared by both types | content-layer-browse-surface, content-layer-add-flow |
| History, Updates | Takeover of the screen | Both tabs render one shared screen over the recents engine, which owns assembly, search, selection, the dialogs and the action verbs; the four feed models stay live behind adapters. The two tabs keep only what needs the host: the badge reset, the splash gate, the bottom nav and reselect. Still forked: the filter sheet, which writes the Updates surface's keys whatever opened it | content-layer-recents-surface |
