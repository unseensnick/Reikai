...
| Downloads | Takeover of orchestration, in progress | One queue engine over two adapters owns the list, its saved order, the cards, the per-series chapter sheet and the queue actions; `DownloadManager` and `NovelDownloadManager` stay separate engines running side by side. Mihon's queue model and its RecyclerView are deleted and manifested | content-layer-download-surface |
