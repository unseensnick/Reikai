...
| History, Updates | UI leaf only on screen, takeover built behind it | What renders is still shared row composables over per-type feeds, with the behavior forked. Everything replacing that exists and is unrendered: the data floor, the engine, the seam, the render policies and the shared screen. Nothing changes for a user until the cutover points both tabs at it | content-layer-recents-surface |
