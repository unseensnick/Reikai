...
| Library | Takeover of orchestration | Shared engine owns assembly, selection and the action verbs. `LibraryViewModel` is **live**, 888 lines, still the manga provider: the amendment's plan to delete and manifest it has not shipped, and the surface doc says it stays live as an engine file. The two docs disagree; unresolved | content-layer-library-surface |
