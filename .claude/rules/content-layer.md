...
| Browse | Behavior-partial | Shared bulk-favorite generic, shared dialogs and the default-category kernel. Verbs stay per-type and the pager and filter dispatch stay declined on mechanism grounds. The add decision and the write order are now shared by both types and device-verified; the duplicate dialog is still twinned | content-layer-browse-surface, content-layer-add-flow |
