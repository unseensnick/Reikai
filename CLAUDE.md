...
**Watch item:** a model resolved by a bare `viewModel<T>()` must not be `private` (crashes on open, every build type, debug included). No screen is on `ScreenModel` any more and `voyager-screenModel` is out of the build. Record: [docs/dev/plans/viewmodel-migration.md](docs/dev/plans/viewmodel-migration.md).
...
**The top of [ROADMAP.md](ROADMAP.md) is the only place the cut's gate list lives** (owner rulings 2026-08-21 and 2026-08-25); it currently says nothing gates 0.4.0 and when to cut is the owner's call. Read it there rather than restating it here, so the two cannot disagree. Nothing else moves the cut.

**0.3.0, 0.3.1 and 0.3.2 have shipped**, tagged and moved to [docs/dev/shipped.md](docs/dev/shipped.md); 0.3.2 is the cut that renamed the app to `app.reikai`. `app/build.gradle.kts` reads `versionName 0.3.2` / `versionCode 196` (the `versionCode` climbs mid-cycle whenever a preference migration needs it, see below; `versionName` moves at the 0.4.0 cut). Notes for continuing sessions:

- New work lands its CHANGELOG entries under `[Unreleased]` and normally bumps nothing: `versionCode` / `versionName` move only at release-cut (see the `feedback_version_bumps` memory). **Standing exception:** a version-gated data migration is a no-op until the shipped `versionCode` reaches its gate, so adding one bumps `versionCode` mid-cycle to make it fire in dev / preview builds and be testable. That is why this cycle already climbed from 0.3.2's `185` to `196`. `versionName` stays `0.3.2` until the cut.
...
- **Each preference migration gates on its own `versionCode`, never a reused one.** The current top is `FlareSolverrCredentialsMigration` (`version = 196f`) on `versionCode 196`; grep `override val version` under `mihon/core/migration/migrations/` for the ones below it. A new migration gates on 197+ and bumps `versionCode` to match.
...
- [.claude/rules/roadmap-plans.md](.claude/rules/roadmap-plans.md): ROADMAP.md, docs/dev/plans/, shipped.md and parked.md structure and naming (path-scoped; loads when editing those files).
...
- [../Reikai-website/UPSTREAM-SYNC.md](../Reikai-website/UPSTREAM-SYNC.md): the **docs site's** sync ledger, which lives in the website repo rather than here because the site is a separate repository with its own upstream and its own cadence. Records the compared-against SHA, what was taken and declined, the gaps still open, and where Reikai is deliberately ahead of Mihon's site (the download page's asset handling and its failure mode; don't port those backwards). `docs/dev/upstream-sync.md` is the app's half and points here.
- **Doc flow (finish an item, then ship):** [ROADMAP.md](ROADMAP.md) forward backlog, then [CHANGELOG.md](CHANGELOG.md) `[Unreleased]` (the source of truth for release notes, benefit-first bold headline), then [docs/dev/shipped.md](docs/dev/shipped.md) done-log at release-cut. CHANGELOG format: `.claude/rules/workflow.md`; ROADMAP, shipped.md and parked.md format: `.claude/rules/roadmap-plans.md`.
...
- **Read-only reference clones live in the `refs/` directory, which is a SIBLING of this `app/` repo, not inside it.** The repo root is `E:\Code\yokai-y2k\app`; the clones are at `E:\Code\yokai-y2k\refs` (i.e. `../refs/<name>` from the repo root, declared as absolute paths in `.claude/settings.json`). `refs/foo` relative to the app cwd does NOT resolve; use `../refs/foo` or the absolute path. The sync / port sources and what each is for:
  ...
  - `refs/mihon-website/`: **Mihon's docs site** (`mihonapp/website`), the upstream for Reikai's own site at `../Reikai-website`. Everything lives under `website/` in that clone, so their pages are `refs/mihon-website/website/src/`. Ported by hand; the ledger sits with the site, at [../Reikai-website/UPSTREAM-SYNC.md](../Reikai-website/UPSTREAM-SYNC.md).
  ...
  - `refs/lnreader-main/` (+ `refs/lnreader-2.0.3-Pre-release/`, `refs/lnreader-plugins/`): **LNReader**; the origin of the retired **novel reader** engine (the vendored `core.js`, deleted at the reader cutover) and of the LN plugin ecosystem.
  - `refs/tsundoku/`: **Tsundoku**, an Apache-2.0 Mihon fork built for novels; the reference for **novel-reader features**, and the source of the native text renderer now in the shared reader. See [docs/dev/plans/novel-reader-tsundoku.md](docs/dev/plans/novel-reader-tsundoku.md).
