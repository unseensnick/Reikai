...
Reikai is collapsing the near-duplicate manga and novel stacks into one Reikai-owned layer, one surface at a time. The UI-leaf seam shipped (History + Updates rows, cover dialog, details screen, library, browse/search, notes, track dialog; recorded in [docs/dev/plans/unified-content-ui.md](docs/dev/plans/unified-content-ui.md)). The forward direction goes deeper: a shared behavior + UI layer over a neutral `Entry` vocabulary with thin Manga/Novel adapters (the manga `ScreenModel` stays live and synced, the novel one dissolves), a delete-and-manifest dead-file policy replacing the retired keep-inert rule, and the tsundoku reader migration as the final phase. One scoped amendment (2026-07-22): on the **library** surface only, the shared layer also owns selection and pipeline orchestration, since the All chip's mixed list (shipped 2026-07-31) makes a selection neither `ScreenModel` can own. The manga model stays live as the provider; the rule is unamended elsewhere. Read [docs/dev/plans/content-layer-architecture.md](docs/dev/plans/content-layer-architecture.md) first for anything forward-looking; it is the authoritative design.
