...
class LnRegistryTest {
    ...
    fun `unknown top-level fields are tolerated`() {
        // Upstream may add fields (e.g. `total`, deprecation flags) that we don't model yet.
        // Parsing must not break in that case.
        val entries = LnRegistry.parse(
            """[{
                "id": "future",
                "name": "Future Source",
    ...
    fun `missing required field throws SerializationException`() {
        // `url` is required (every upstream entry has one; we'd be unable to install otherwise).
