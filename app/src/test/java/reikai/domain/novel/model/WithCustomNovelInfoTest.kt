...
class WithCustomNovelInfoTest {

    private val source = Novel.create().copy(
        id = 1L,
        title = "Source Title",
    ...
    fun `an all-null overlay passes every field through`() {
        source.withCustomInfo(CustomNovelInfo(novelId = 1L)) shouldBe source
    }

    @Test
    fun `a set field wins over the source value`() {
        source.withCustomInfo(CustomNovelInfo(novelId = 1L, title = "My Title")).title shouldBe "My Title"
    }

    @Test
    fun `an unset field keeps the source value even when another is overridden`() {
        val custom = CustomNovelInfo(novelId = 1L, title = "My Title")
        source.withCustomInfo(custom).author shouldBe "Source Author"
    }

    @Test
    fun `every field can be overridden at once`() {
        val custom = CustomNovelInfo(
            novelId = 1L,
            title = "T",
            author = "Au",
            artist = "Ar",
