 ...
 * source via [GalleryAdder], then routes to the reader (if the link was a chapter) or the manga
 * details page.
 */
class InterceptActivity : BaseActivity() {
            ...
            Column(
                ...
                when (status) {
                    InterceptResult.Idle, InterceptResult.Loading -> {
                        Text(
                            text = stringResource(MR.strings.loading_entry),
                            style = MaterialTheme.typography.titleLarge,
                        )
                        CircularProgressIndicator(modifier = Modifier.size(56.dp))
                    }
                    is InterceptResult.Success -> Text(
                        text = stringResource(MR.strings.launching_app),
                        style = MaterialTheme.typography.titleLarge,
                    )
                    is InterceptResult.Failure -> Text(
                        text = stringResource(MR.strings.error_with_reason, status.reason),
                        style = MaterialTheme.typography.titleLarge,
                    )
                }
            }
                ...
                when (it) {
                    ...
                    is InterceptResult.Failure -> {
                        MaterialAlertDialogBuilder(this)
                            .setTitle(stringResource(MR.strings.chapter_error))
                            .setMessage(stringResource(MR.strings.could_not_open_entry, it.reason))
                            .setPositiveButton(stringResource(MR.strings.action_ok), null)
            ...
            if (sources.size > 1) {
                withUIContext {
                    MaterialAlertDialogBuilder(this@InterceptActivity)
                        .setTitle(this@InterceptActivity.stringResource(MR.strings.label_sources))
    ...
    private suspend fun loadGalleryEnd(gallery: String, source: UrlImportableSource? = null) {
        val result = galleryAdder.addGallery(
            context = this@InterceptActivity,
            url = gallery,
            fav = true,
            forceSource = source,
        )
