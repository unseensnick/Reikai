...
class MangaDexLoginHelper(
    ...
    // The auth endpoints reject Reikai's browser User-Agent (HTTP 400 + the MangaDex web page), so
    // the OAuth calls must carry the delegate's extension headers, the same fix MangaDexService uses.
        ...
        val error = kotlin.runCatching {
            client.newCall(
                POST(
                    ...
                    headers = headers.newBuilder().add("Authorization", "Bearer $sessionToken").build(),
                    body = formBody,
                ),
            ).awaitSuccess()
            mdList.logout()
