...
// RK: inert. The track dialog was unified into the shared reikai.presentation.track.EntryTrackInfoDialog
// (ported from this file, serving manga + novels), so these screens have no call sites; the details screens
// push EntryTrackInfoDialogHomeScreen instead. Left in place, not deleted: it is the upstream-sync reference
// to diff future Mihon track-dialog changes into EntryTrackInfoDialog. Keep it verbatim upstream, so the
// diff stays honest; Reikai behaviour belongs in EntryTrackInfoDialog, never here.
