 ...
 * Runs just before a split rather than at merge time: while merged, [GetTracksInGroup] already reads the
 * whole group, so a copy made earlier would only sit there going stale. Gated by
 * [ReikaiLibraryPreferences.syncTrackerLinksGrouped].
 *
 * Each member ends up with its own `manga_sync` row carrying the group's furthest-read values: a member
 * left behind at chapter 5 by an older build would otherwise push the remote service backwards after the
 * split. A tracker with conflicting remote ids across the group is skipped rather than guessed at.
 */
