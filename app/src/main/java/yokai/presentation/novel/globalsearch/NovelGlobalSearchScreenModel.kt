 ...
 * progressively (mirrors the legacy manga `GlobalSearchPresenter`). Hosted via a Conductor bridge
 * ([eu.kanade.tachiyomi.ui.novel.globalsearch.NovelGlobalSearchController]); routing to details
 * reuses the same push as browse.
 */
    ...
    init {
        ...
        screenModelScope.launchIO {
            novelRepository.getLibraryNovelAsFlow().collectLatest { library ->
                val keys = library.mapTo(HashSet()) { it.novel.source to it.novel.url }
