...
/**
 * Compose markdown renderer for manga descriptions. Ported from Mihon; mirrors its theming, with the
 * project spacing values inlined (we have no `MaterialTheme.padding` extension). The flavour strips
 * raw HTML and reference links and adds GFM tables ([SimpleMarkdownFlavourDescriptor]).
 */
@Composable
fun MarkdownRender(
    content: String,
