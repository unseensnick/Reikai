 ...
 * stub-source machinery: lnreader plugins are JS files we load directly, not APKs we class-load,
 * so there is no "source installed but binary not loaded yet" state to model.
 *
 * Slice A is in-memory only. Phase 3 step 5 (the add-repo screen) will wire a persistent
 * installer that registers sources on app start.
 */
