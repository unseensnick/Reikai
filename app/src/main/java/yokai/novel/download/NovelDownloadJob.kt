 ...
 * survive backgrounding; WorkManager re-runs it after a restart, where [NovelDownloadManager]
 * restores the persisted queue and resumes. Sibling of the manga
...
class NovelDownloadJob(context: Context, workerParams: WorkerParameters) :
    ...
    private val manager: NovelDownloadManager by inject()
    ...
    companion object {
        ...
        /** Start (or reuse) the downloader. KEEP so adding chapters to a running drain doesn't
         *  restart it; the running loop picks up the newly-queued items. */
