...
## Manga and novel categories

**Reikai** keeps a separate category list for each library. In <nav to="categories">, the **Manga** / **Novels** chip at the top switches between them, and creating, renaming, hiding, reordering or deleting a category only affects the list you are viewing.
...
This avoids deleting one at a time when cleaning up a long list. A selection covers only the list you are viewing, manga or novels.
