...
- **Source-scoped reading: planned, then superseded.** The reader always spans the group, so opening a chapter from a per-source chip on the details screen still shows the whole group's chapter list. A fix was fully scouted (an Intent extra carrying the chip selection, applied to `chapterList` only, deliberately leaving `mergedGroup` full so the group-wide duplicates-read pass in `unfilteredChapterList` kept working). It was dropped in favour of [rebuilding the merge system](merge-system-rebuild.md): the reader cannot tell "the group" from "this source" because a group has no identity to open, so the extra was a workaround for the real gap. Revisit only if the rebuild stalls.
