...
- **Reuse, don't duplicate.** Group resolution (via `MangaMergeManager`, now backed by the persisted `MergeGroupRepository` after the [merge system rebuild](merge-system-rebuild.md)), chapter stitching (`ChapterAggregation`), and the chapter row (`MangaChapterListItem`) are all reused; the only new shared home is the aggregate + reading-order policy, now called by both reader and details.
