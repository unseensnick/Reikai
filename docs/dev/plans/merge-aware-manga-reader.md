...
- **Unified list.** `MergedChapterProvider.load(anchorManga)` resolves the merge group (`mergeManager.computeRelatedIds`) and returns the unified chapter list rendered from the group's stored stitch (`stitchOf`, then `merged`, which calls the shared `renderStoredStitch`), the group's manga keyed by id, and each member's source name. For an unmerged manga it returns exactly the single-source list (zero change for the common case). The details screen's merged-chapters flow reads the same `stitchOf` and `merged`, so reader and details cannot disagree.
...
- **Per-chapter incognito.** Whether progress, history, the read mark and tracker sync are written is asked for each chapter written, from its own source, through the `ChapterIncognito` kernel the novel reader shares. Upstream reads it once from the opened series, which a merged group's members can contradict, and a seamless window writes a neighbour's progress too.
- **Per-source labels.** The in-reader chapter list (the shared `ReaderChapterListDialog`, fed rows by `MangaReaderProvider`) shows each chapter's source name in its subtitle, the same way the novel reader's sheet does; each row also carries its own source's manga so the row's download indicator is correct.
...
- `app/src/main/java/reikai/domain/manga/MergedChapterProvider.kt` (new): group resolve + unified list + source names; `stitchOf` and `merged` read the stored stitch.
...
- `app/src/main/java/reikai/presentation/reader/ReaderChapterListDialog.kt` + `.../ui/reader/chapter/ReaderChapterItem.kt`: per-source label in the row subtitle.
...
- `app/src/main/java/eu/kanade/tachiyomi/ui/manga/MangaViewModel.kt` (`// RK`): the details merged-chapters flow calls `MergedChapterProvider.stitchOf` and `merged`.
- `app/src/main/java/reikai/domain/merge/OpenedChapter.kt`: `withOpenedChapter`, shared by both readers.
...
- **Reuse, don't duplicate.** Group resolution (via `MangaMergeManager`, now backed by the persisted `MergeGroupRepository` after the [merge system rebuild](merge-system-rebuild.md)), chapter stitching (the stored stitch, built by `MangaGroupStitcher` over `ChapterAggregation`), and the chapter sheet are all reused; reader and details read the same stored stitch.
...
- **Externally-opened chapters** (history, updates, or a non-preferred source's chip) that were deduped out of the unified list are appended so they still open, via the top-level `withOpenedChapter` (`reikai/domain/merge/OpenedChapter.kt`, shared with the novel reader), which **restamps the combined list**. The re-added row still carries its own source's `sourceOrder` while the unified list was renumbered to one 0..N-1 scale, and the reader sorts on `sourceOrder` alone (`ChapterSort`), so appending it raw dropped it at an arbitrary index and broke prev/next. Nothing is restamped unless a chapter is actually added, so an unmerged manga keeps its source's own ordering.
...
- **Source-scoped reading: delivered by the merge rebuild.** The reader now opens either the whole group or one source's own list, chosen by the entry point: Details / Library / History open group scope; Updates, a per-source chip, and new-chapter notifications open source scope. The signal is a `source_scoped` boolean Intent extra; the group is still resolved from the entry id, and read-state propagation stays group-wide so marking read still de-dupes across sources in either scope. This is the scope-by-context design from the [merge system rebuild](merge-system-rebuild.md) (Phase 3), which resolved the earlier gap (the reader could not tell "the group" from "this source" because a pref-derived group had no identity to open). The mechanism this reader layer got right (scope `chapterList`, keep `mergedGroup` full so the group-wide duplicates-read pass still works) carried over.
