...
**Queue status.** Whether the queue is downloading, paused or stopped is one rule, `downloadQueueState` in `DownloadQueueKernel.kt`, which the More row and the queue screen's pause button both read: Downloading while any downloader with something queued is running, else Paused while anything is queued. The More row used to count every queued novel as downloading, so a paused novel queue read Downloading there while the queue screen offered Resume.
