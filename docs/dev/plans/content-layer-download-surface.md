...
In progress. Steps 1 and 2 landed together (`c62b43ef7`), since the engine has no screen to verify on without the new list. Step 3's forced failure was shown for manga only: the novel plugins tried return a page even for an unreachable or malformed chapter URL, so a novel failure reason is verified from code, not on screen. Sequence, each step its own commit and each gated on its check:
