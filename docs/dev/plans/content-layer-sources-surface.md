...
The `tachiyomix.*` keys are shared by the first two. The `Extension` model carries the kind; `AndroidSourceManager` skips novel kinds before it registers anything, so no manga stub rows are written; the install receiver and private installs accept all three. Trust is unchanged: a store's signing key covers its extensions, and a keyless store's extensions are trusted per version by the user, which is where every IReader extension lands because its index carries no key.
...
- **A2, the tachiyomi-format kind in the pipeline.** Feature, keys and gate in `ExtensionLoader`, the kind on `Extension`, `AndroidSourceManager` skipping it, receiver and private install. Verify: install a NovelSourcery APK; it loads as a novel extension and no manga source or stub appears.
- **A3, stores.** `isNovel` (proto field 8000) on `NetworkExtensionStore`, the extension list split by content type, store kinds labelled on the Repos screen. Verify: the NovelSourcery repository lists 149 novel extensions and nothing in the manga list.
