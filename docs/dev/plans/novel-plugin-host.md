...
- **Polyfill completeness over a generic browser shim.** QuickJS lacks the browser globals the WebView gave for free, and the host only polyfills what shipped plugins actually use (audited list in the `ln-host-polyfill-parity` memory; `atob`/`btoa`, `Blob` and `Buffer` have since joined the shipped set). Deliberately omitted because no shipped plugin uses them: `document`, `navigator`, a real `XMLHttpRequest`, a global `crypto`. The tradeoff is that adding a plugin that uses a missing global means adding a shim, but the alternative (shipping a full browser emulation) is far more surface to maintain for features nobody calls.
...
- **One app-scoped host, one QuickJS engine per plugin.** Two QuickJS natives cannot coexist (both `libquickjs.so`), so manga's JavaScript runs on the same dokar engine through `app.cash.quickjs.QuickJs`, a shim over the Cash API extensions compile against, and `JavaScriptEngine` keeps Mihon's body over that shim. The residual risk is behavioural: dokar returns other shapes than Cash (a JS array arrives as a `List`), so the shim converts arrays back to `Object[]` (`CashValueTest`, and the instrumented `appCashQuickJsCompatShimWorks`); objects stay dokar's `Map`, since Cash never returned one. Within the host, each plugin runs in its own engine on its own confinement thread (created lazily, idle-closed after 60s), so calls to different sources run in parallel while calls to one plugin still serialize behind that plugin's mutex, the correctness requirement that makes its globals safe. A shared loader engine handles bulk info extraction at app start so `ensureLoaded` does not spike one engine per installed plugin. The per-source isolation follows tsundoku's shape; the earlier single shared engine serialized all plugin traffic app-wide through one lane.
...
- **A plugin's update comes from the repo still listing its installed address (owner, 2026-09-18).** `findPluginUpdates` (`LnPluginUpdateChecker.kt`) first looks for a repo whose entry for the plugin id is at the installed URL, and that entry decides; only when no repo lists the installed URL does the highest version across repos win, with a tie going to the earlier repo. Accepting another repo's entry makes `installFromUrl` delete the installed script and repoint the install at that repo, and the listing is the only record of where a plugin came from, since `LnInstalledPluginMetadata` stores no repo. The cost, accepted: a newer build of the same id in another repo is not offered while the source repo still lists the plugin. Revive only if moving plugins between repos becomes wanted.
