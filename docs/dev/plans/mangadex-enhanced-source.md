...
**IN PROGRESS.** On branch `feat/mangadex-source` off `main` (not `roadmap/later`). Phase 0 shipped; Phase 1 foundation shipped; a 5-agent deep-research pass mapped the whole port's blast radius (see the corrections folded through this doc). This doc is the forward plan; each phase's notes get rewritten to "what shipped" (with commit SHAs) as it lands. Size: `[L]`, multi-session. The `exh/md` payload is ~37 files / ~3,535 LOC in Komikku plus the wrapper, tracker, metadata, and settings glue.
...
Acceptance: install the MangaDex extension on-device, toggle delegate on, browse/search/open details/read a chapter through the enhanced source; confirm parity with the raw extension. **Dump the installed MangaDex source name + id; confirm the per-language sources are named `"MangaDex"` (so the name-match wraps them) and the id is in `MANGADEX_IDS`** (the Comick lesson). Run a minified `:app:assemblePreview` (first Injekt generics via `getEnabledMangaDex`).

**Phase 2, details rendering (metadata UI only). IMPLEMENTED + compiles (`a39cb5752`); on-device verification pending.** The metadata is already parsed, namespaced, and stored in Phase 1 (`MetadataSource` + `ApiMangaParser.parseIntoMetadata`); Phase 2 just renders it. What shipped:
...
**No `GalleryInfoBox` branch added (deliberate, deviates from the earlier "add a MangaDex branch" wording).** In Reikai `getExtraInfoPairs` feeds two consumers: the inline card's `GenericGalleryInfo` (filters `http`-valued rows, so cover/uuid stay out) and the full `MetadataViewScreen` "more info" viewer (shows everything). The E-Hentai curated branch (`EHentaiGalleryInfo`) exists only because EH data is visually rich (rating stars, genre colours, flags); MangaDex's scalar rows render fine and consistently through the generic path (same as nHentai/Pururin), so a bespoke branch would be inconsistent and is YAGNI. The empty-card gate (`6452ae555`) flips on automatically now that the pairs are non-empty. A curated branch can still land later if wanted.
...
**Phase 3, OAuth login + MDList tracker.** Port `MangaDexAuthInterceptor` (OAuth2 PKCE: bearer header, refresh-on-401), `MangaDexLoginHelper`, `MangaDexLoginActivity` (extend `BaseOAuthLoginActivity`), `MangaDexAuthService`; un-defer `MdUtil`'s OAuth methods + the `MdApi`/`MdConstants` auth members. Store the OAuth blob in `TrackPreferences.trackToken(mdList)`. Port `MdList : BaseTracker` (id `60L`, confirmed free) and register in `TrackerManager`. **Coupling (deep research):** `MdList.bind/refresh/update` proxy into the source's `fetchTrackingInfo`/`updateFollowStatus`/`updateRating`, so this phase also needs `FollowsHandler`'s per-title tracking subset (not just OAuth), and `MangaDex.kt` gains `LoginSource` (net-new interface). `MdList` throws `MangaDexNotFoundException` if no MangaDex source is enabled. **OAuth scheme decision:** keep Komikku's `tachiyomisy` client id + `tachiyomisy://mangadex-auth` redirect and add that intent-filter to `AndroidManifest.xml` (Reikai's trackers share a `mihon://` `TrackLoginActivity`; a `mihon://mangadex-auth` scheme would need a newly registered MangaDex OAuth client). Adds `md_follows_unfollowed` i18n. Acceptance: log in, MDList binds a title, status/rating round-trips (device-verified against a real MangaDex account).
...
- **Minified build.** The net-new `exh.md` package uses Injekt generics (first via `MdUtil.getEnabledMangaDex` in Phase 1); it is under the existing `exh.**` keep, but verify a minified `:app:assemblePreview` at Phase 1 (Phase 0 had no Injekt generics, so it was skipped there).
