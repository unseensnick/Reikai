...
**Removed** in commit `ee9f91749`, after 0.3.2. The importer only ever read the app's own `tachiyomi.db`, and 0.3.2 renamed the app from `eu.kanade.tachiyomi.y2k` to `app.reikai`. Android keys an app's data folder, updates and backup restores to its package, so a Yōkai-era install now meets a Reikai build only as a separate app with a fresh, Mihon-schema database, and the importer could never fire. Every release from 0.1.2 through 0.3.1 still ships it under the old id, so a Yōkai user who updates in place to one of those is still recovered. The move to `app.reikai` goes through backup and restore, the steps the 0.3.2 release notes carry. The files listed under Key files are deleted.

Previously: shipped in 0.1.2. Verified on the emulator end to end, both with a synthetic Yōkai DB and with a real Yōkai-era backup: the crash reproduces on 0.1.0, and updating to the fixed build recovers the library (266 manga and 2 extension repos in the real-data run) with chapters, read state, categories, history, and tracking; the old DB is preserved as `.yokai.bak`; a fresh DB is created (`extension_store` present, no crash); and the library renders flat, every entry visible in its categories with no manga hidden by stale merges.
...
- **Keep the importer; elapsed time is not draining the population it serves** (owner, 2026-08-20). The usual argument for retiring a migration path is that people update and the old install base shrinks. That does not hold here, because the Yōkai builds cannot be updated from inside the app. Their checker is `Version.parse(newVersion) > Version.parse(currentVersion)` inside a `catch (IllegalArgumentException) { false }`, so an install on `1.9.7.5.9` offered tag `v0.3.1` either compares 0 against 1 and says no, or fails to parse five segments and says no. No update is ever offered. The only route onto the Mihon line is finding the repo and downloading by hand. Meanwhile the final Yōkai release still has 326 downloads, more than any release in the repo (v0.3.0 has 295, v0.3.1 has 188), so the stranded population is real and static rather than shrinking. Removing the importer would hand every one of them the original crash on their eventual upgrade, with an unrecoverable library. Revisit only after deliberately draining them, for example a final Yōkai-tagged release whose only change is an updater pointing at the new line. **This decline expired with 0.3.2:** its premise was that the eventual upgrade lands in place on the same package, and after the rename it cannot (see Status).
- **A category's sort comes from its order letter, never its `flags`.** Yokai never read the `flags` column; each category's sort lived in `manga_order` / `novel_order` as one letter, `'a' + 2 * catValue` plus one for descending, with `D` or a slash-separated id list meaning drag-and-drop (`Category.mangaOrderFromString`, `LibrarySort.categoryValue` on `design/library-compose`). A letter becomes the category's own sort with the override bit set, because every Yokai category had its own sort; the type is the one Yokai's own `LibrarySort.serialize()` names. Drag-and-drop and a blank follow the global sort, since Reikai cannot hold a manual order. Direction depends on which library drew the letter: the shipped manga library (`LibraryPresenter.applySort`, v1.9.7.5.9) ran Latest chapter, Last read, Total chapters, Date added and Date fetched newest or largest first on an even letter, while the branch's Compose libraries (`MangaLibrarySort`, `NovelLibrarySort`) did not. Manga follows Yokai's `pref_use_compose_library` switch, which survives the in-place update; novels only ever rendered through `NovelLibrarySort`.
