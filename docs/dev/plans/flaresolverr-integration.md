...
- **The proxy sign-in reaches only the saved server** (owner, 2026-09-23). `flareSolverrLoginFor` adds it only for the saved address's scheme, host and port, and only over https or the user's own network (`isPrivateChannel`, shared with the forwarded cookies); the solver's client follows no redirect, which OkHttp would follow without the header and so report as a wrong password. Removing the header at the proxy is the reader's side, documented per proxy in `docs/flaresolverr.md`, from each proxy's own documentation read the same day.
