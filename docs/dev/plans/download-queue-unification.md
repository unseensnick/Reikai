...
- **Download error notifications follow Mihon on "Hide notification content" (owner ruling, 2026-09-18).** Mihon's `DownloadNotifier.onError` names the entry and chapter even with that switch on, so both downloaders' errors keep the title under it. Reikai's "Hide adult content in notifications" does apply, to manga and novels alike: an adult entry's error takes the generic downloader title and drops the chapter name too, since one can name the series. The rule is `downloadErrorTitle` in `reikai/data/notification/ShownEntryName.kt`; manga's adult verdict is the same `AdultContentChecker.adultIdsAmong` its progress notification asks. The novel downloader's paused lines (no network, Wi-Fi only) are status text rather than an entry name, so they show under either switch, as manga's pause notice does.
