...
The install/manage/update UI lives on the **Extensions** tab, mirroring how manga extensions are installed there (not on Sources, which only browses what is already installed). `LnPluginManagerViewModel` ([LnPluginManagerViewModel.kt](../../../app/src/main/java/reikai/presentation/browse/extension/LnPluginManagerViewModel.kt)) renders the same three sections  as Mihon's `ExtensionsViewModel`: Updates, Installed, and Available. It fans `fetchRepo` across every added repo concurrently as the single source of network truth, diffing versions to fill both the Available list (repo minus installed) and the Updates list, and writes the pending-update count back to preferences so the tab badge can show it.

Repos for both manga and light novels are managed in one Reikai-owned `RepositoriesScreen` ([RepositoriesScreen.kt](../../../app/src/main/java/reikai/presentation/browse/repos/RepositoriesScreen.kt)), which replaced Mihon's `ExtensionStoresScreen`. The Extensions overflow opens it via a single "Repos" action.

### How Browse hosts novel sources

The Browse tab keeps its existing Sources and Extensions sub-tabs. A sticky `All / Manga / Novels` chip at the top of each decides what that tab shows, as a predicate over one list the shared engine assembles from a manga and a novel provider (`SourcesEngine`, `ExtensionsEngine`); All shows both with a small content-type badge per row. The takeover that built this is recorded in [content-layer-browse-surface.md](content-layer-browse-surface.md), which is the live record for everything below the chip.

`BrowseTab` ([BrowseTab.kt](../../../app/src/main/java/eu/kanade/tachiyomi/ui/browse/BrowseTab.kt)) swaps Mihon's `sourcesTab()` / `extensionsTab()` for `reikaiSourcesTab()` / `reikaiExtensionsTab()` inside a `// RK` island. A shared `ReikaiBrowseViewModel` ([ReikaiBrowseViewModel.kt](../../../app/src/main/java/reikai/presentation/browse/ReikaiBrowseViewModel.kt)) holds the one sticky `browseContentType` preference (so both sub-tabs stay in sync) and the LN update count that feeds the combined Extensions badge (manga updates plus LN updates). It kicks the cache-gated `LnPluginUpdateChecker.runIfStale()` on Browse open so the badge is fresh without the user touching the Novels chip.

On the Sources tab, the Novels branch is `NovelSourcesViewModel` ([NovelSourcesViewModel.kt](../../../app/src/main/java/reikai/presentation/browse/source/NovelSourcesViewModel.kt)): it calls `ensureLoaded()`, follows `NovelSourceManager.sources`, and groups sources by language with a leading Pinned section, exactly like the manga sources list. Pinning toggles `pinnedNovelSources` ([ReikaiSourcePreferences.kt](../../../app/src/main/java/reikai/domain/source/ReikaiSourcePreferences.kt)). Tapping a source opens its per-source browse and records it as last-used (skipped while Incognito).
...
- [reikai/presentation/browse/ReikaiBrowseViewModel.kt](../../../app/src/main/java/reikai/presentation/browse/ReikaiBrowseViewModel.kt): shared content-type chip state + combined update badge.
- [reikai/presentation/browse/source/ReikaiSourcesTab.kt](../../../app/src/main/java/reikai/presentation/browse/source/ReikaiSourcesTab.kt) + [NovelSourcesViewModel.kt](../../../app/src/main/java/reikai/presentation/browse/source/NovelSourcesViewModel.kt): Sources tab wrap + the novel sources list (language groups, Pinned section, pin toggle).
- [reikai/presentation/browse/extension/ReikaiExtensionsTab.kt](../../../app/src/main/java/reikai/presentation/browse/extension/ReikaiExtensionsTab.kt) + [ExtensionsEngine.kt](../../../app/src/main/java/reikai/presentation/browse/extension/ExtensionsEngine.kt) + [LnPluginManagerViewModel.kt](../../../app/src/main/java/reikai/presentation/browse/extension/LnPluginManagerViewModel.kt): Extensions tab, the shared list engine, and the plugin half's Updates / Installed / Available state.
...
- [reikai/presentation/browse/catalogue/EntryCatalogueScreen.kt](../../../app/src/main/java/reikai/presentation/browse/catalogue/EntryCatalogueScreen.kt) + [NovelBrowseViewModel.kt](../../../app/src/main/java/reikai/presentation/novel/browse/NovelBrowseViewModel.kt) + [NovelSourceFilterSheet.kt](../../../app/src/main/java/reikai/presentation/novel/browse/NovelSourceFilterSheet.kt) + [NovelSourceSettingsSheet.kt](../../../app/src/main/java/reikai/presentation/novel/browse/NovelSourceSettingsSheet.kt): the shared per-source catalogue, the novel model behind it, and the plugin filter and settings sheets.
...
- `Notifications.kt`: the LN update channel. The update job is scheduled by `mihon/core/migration/migrations/SetupLnPluginUpdateMigration.kt`.
...
- **One repo screen for both kinds.** Manga extension stores and LN plugin repos share `RepositoriesScreen`; the decision is recorded in [content-layer-browse-surface.md](content-layer-browse-surface.md).
...
- **Self-healing load over a hard install state.** `ensureLoaded()` retries failed loads on each novel-screen open, so a transient download failure recovers without a cold restart. A plugin whose load fails is still recorded, though: `LnPluginInstaller.failures` holds the reason until a load, install or uninstall clears it, and the Extensions list shows it under Not loaded with the same reason dialog and uninstall a manga extension gets (owner, 2026-09-15, ported with mihonapp/mihon#3953). Retrying heals a transient failure; the record is what makes a deterministic one visible and removable.
