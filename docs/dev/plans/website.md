...
- **The landing page carries four feature cards, chosen against the README's own claims** (owner, 2026-08-26). The README names two things that set Reikai apart, one library for both content types and the multi-source features, and those hold the first two slots. The other two went to the taste profile and the Cloudflare bypass, because each answers a different question a visitor arrives with: what will it show me next, and what happens when a source breaks. Novel tracking and library search lost their slots, the first being a sub-claim of card one and the second not appearing in the README's feature list at all. Adult sources was never in the running: the landing page is exactly the marketing surface the public-facing naming rule covers, even though the doc itself publishes.
- **The privacy page names no tracking services.** It listed six while the app supported nine, and would fall behind again on the next one. It now states the rule and links the tracking guide, which owns the list, and separates the three self-hosted services, which is the part that bears on privacy at all.
...
- **Two site pages carry feature copy the docs sync never touches**: the landing page's feature cards and the privacy policy's account of what leaves your device. Both live in the site repo and are hand-maintained, so a capability that lands in the app and updates a doc here leaves those two stale with the build still green, since neither is a link. Check them whenever a user-visible capability ships.
