...
**Incognito mode for novels.** Novels honor the global Incognito toggle. While incognito, the novel reader does not record history, scroll position, read state, or last-read, and skips the reader's tracker push; the browse model does not record the last-used novel source. Incognito is per source for novels too, as for manga: `GetIncognitoState` takes a `SourceKey`, and a plugin is stored in the incognito set under its `SourceKey` form. The reader asks for each chapter it writes, from that chapter's own source, through the `ChapterIncognito` kernel both readers share, so a merged novel's members can differ. The details mark-read tracker sync stays ungated, matching manga.
...
- **The details mark-read tracker sync stays ungated by incognito, matching manga.** Incognito is per source for both types and decided per chapter in the reader (see Incognito mode for novels above).
