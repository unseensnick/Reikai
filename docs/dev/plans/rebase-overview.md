...
- [.claude/rules/](../../../.claude/rules/): the working rules: [architecture.md](../../../.claude/rules/architecture.md) (Compose + Voyager, Injekt DI, preferences, coroutines, modules), [compose-port.md](../../../.claude/rules/compose-port.md) (the screen conventions a ported screen must follow), [workflow.md](../../../.claude/rules/workflow.md) (changelog, commits, upstream-sync method), plus [code-quality.md](../../../.claude/rules/code-quality.md), [testing.md](../../../.claude/rules/testing.md), [database.md](../../../.claude/rules/database.md), and [security.md](../../../.claude/rules/security.md).
...
| P5 | Light-novel vertical (domain/DB, QuickJS host, browse, details, reader, downloads, library, background updates, merge, grouping, unified Updates) | Core shipped; round-2 parity polish ongoing |
...
What remains is the round-2 backlog: a manga/novel feature-parity sweep (for example novel batch migration, the download-queue reorder, the home-screen widget) and the revived adult-source subsystem. The signed release pipeline (AGP-native signing plus the release and preview GitHub Actions workflows, with the in-app updater pointed at Reikai's own release repos) is built and CI-validated. The in-progress and deferred items are tracked in [ROADMAP.md](../../../ROADMAP.md).

## Decisions & tradeoffs

- **Manual port, never `git merge`.** Upstream Mihon changes are ported by hand from `refs/mihon/`. A merge would overwrite Reikai's `// RK` patches and its identity. The cost is manual work on each sync; the benefit is the patch set stays intact and reviewable. Sync commits cite the upstream PR as `mihonapp/mihon#N` (a bare `#N` would link to a Reikai issue).
...
- **Keep the preference-based merge, do not adopt Komikku's database merge.** Reikai's merge stores merge groups in preferences rather than fake parent rows in the database: no schema, trivially reversible unmerge, near-zero coupling to the reader and downloader. Manga groups are stored as comma-joined local ids. Because ids are reassigned on restore, the novel backup path serializes each group as stable `{url, source}` refs and rebuilds it against the restored ids (see [novel-backup.md](novel-backup.md)).
