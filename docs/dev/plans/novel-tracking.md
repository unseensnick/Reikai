...
Bind a light novel to AniList, MyAnimeList, MangaUpdates, or Kitsu from its details screen and keep reading progress in sync, reusing Mihon's existing tracker services; merge-group-aware so a track set on one source advances and shows across the whole merged novel.

## Goal

Give novels the same two-way tracker sync manga already has: from a novel's details screen, sign-in-gated, bind the novel to one of the four supported trackers and push status, chapters read, score, and start/finish dates back to that service. Progress should update automatically as you read, and the binding should behave sensibly across a merged novel's multiple sources.
...
**The `// RK` searchNovel path.** The one place Mihon's trackers needed a novel-specific branch is search. Mihon's manga `search()` filters light novels out of the results (it asks the tracker's API for manga-type entries). So each of the four trackers got a `// RK`-fenced `searchNovel()` method that queries for novel/light-novel entries instead. Everything downstream of the search result (binding, finding, updating, OAuth) is unchanged Mihon code.
