...
**The Compose library foundation.** The library is a Voyager `Tab` (`LibraryTab`) backed by Mihon's `LibraryScreenModel`, following the screen conventions in [.claude/rules/screen-conventions.md](../../../.claude/rules/screen-conventions.md) (Voyager `Tab` + `ScreenModel`, state as `StateFlow`, Injekt in the model, coroutines on `screenModelScope`). Reikai does not re-implement the model. Its filter / sort / group / search / badge work all stays in Mihon's `LibraryScreenModel.getFavoritesFlow` / `applyFilters` / `applyGrouping` / `applySort`. Reikai's additions are `// RK`-fenced islands inside that model: the dynamic-grouping branch, the category-reorder step, the hidden-category drop, extra filter dimensions (lewd, category include/exclude), and a parallel state flow off a new `ReikaiLibraryPreferences` holder that carries the roughly twenty display-preference fields Mihon has no equivalent for (hopper visibility / gravity / long-press action, grouping mode, collapsed-category sets, category sort order, panorama and source-badge toggles, and so on). Those preference values are read in the model and exposed as state, never read inside a composable.
