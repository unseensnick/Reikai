...
- [Browse Feed tab (+ saved searches)](browse-feed-tab.md): rows of a source's latest / popular / saved-search results as a Browse surface, ported from Komikku (whose saved-search layer is TachiyomiSY's, absent from Mihon entirely). Staged so saved searches can ship without the feed. Not started.
...
- [Content layer: the library surface](content-layer-library-surface.md): the second surface, collapsing the manga and novel libraries onto the shared content layer. The library UI is already shared (novels ride as a negative-id `LibraryItem`), so the remaining forks are the duplicated `NovelLibraryScreenModel` pipeline and the id-disguise. Phase 1 extracts the shared `buildState` pipeline (and levels the novel lewd filter up), Phase 2 adds the neutral state + adapters + `EntryId`, Phase 3 reconciles the rest. Phase 1 in progress.
- [Merged read state](merged-read-state.md): one definition of read, bookmarked and downloaded across a merge group, so the library badge, the details "All" list, continue-reading, download-next, the reader and history all agree instead of each reading whichever source's copy won the dedup. Designed, not started; sequenced after the library surface.
