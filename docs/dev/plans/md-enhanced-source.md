...
**FEATURE-COMPLETE (2026-07-06).** On branch `feat/enhanced-source` off `main`. Phases 0-5 shipped and on-device verified (delegated wrap + metadata-enriched details + the gallery-info card + OAuth login + the MDList tracker + follows browse with bulk multi-select + the settings hub with two-way follows sync), plus Phase 6's Browse "Random" button (`c6ad2ada7`). The rest of Phase 6 (Similar carousel, external-aggregator handlers, cover / data-saver niceties) was evaluated and deliberately dropped, see Phase 6 below for the rationale. A 5-agent deep-research pass mapped the whole port's blast radius (see the corrections folded through this doc); a later Komikku parity audit added the entries in "Parity-audit additions"; a final path-tracing audit (2026-07-06, see "Post-port audit: Reikai vs Komikku's live bugs") came back clean. This doc is the forward plan; each phase's notes get rewritten to "what shipped" (with commit SHAs) as it lands. Size: `[L]`, multi-session. The `exh/md` payload is ~37 files / ~3,535 LOC in Komikku plus the wrapper, tracker, metadata, and settings glue.
...
**Tracker-search sheet (shipped on this branch).** The MDList search sheet matches the other trackers: covers load through a dedicated Coil fetcher (`MangaDexTrackCoverFetcher`, registered in `App.kt`, wrapped at `TrackerSearch.kt`) that uses the source client + extension headers, because the cover CDN (`uploads.mangadex.org`) 400s the app's browser UA just like the API does. Results carry the full field set (authors, type, year, status, 2-decimal score, description). Speed came from batching: one `getSearchManga` for the hit ids, then one bulk `viewMangas` (with author/artist includes) + one bulk `mangasRating`, run concurrently, instead of a per-result details fetch. Results key on a hashed MangaDex id so `TrackSearch`'s remote-id dedupe doesn't collapse them to one.

**Phase 4, follows browse + bulk multi-select (SHIPPED on this branch, commits `ed9c4a313` + `4ed99e78e`).** Browse the signed-in user's follows and add them to the library. Delivered: the net-new `FollowsSource` source-api interface + `FollowsHandler.fetchFollows`/`fetchAllFollows`/`followsParseMangaPage` + the Voyager follows UI (`MangaDexFollowsScreen`/`ScreenModel`/`PagingSource` under `exh/md/follows/`), reached from a `// RK` Follows button in the browse filter sheet (gated by `getMainSource<MangaDex>()`). The follows model subclasses `BrowseSourceViewModel` (opened up with `// RK` `createSourcePagingSource`/`combineMetadata` hooks), so it inherits the grid, paging and favoriting.

Two corrections to the original plan: `MetadataMangasPage` was **not** net-new (it shipped with the adult-browse-parity work, and the base paging already reads its cursor), so follows just returns it; and follows is **offset-paged, not worker-driven**, with a fixed latent Komikku bug (it stepped the offset by 20 while requesting 100, dropping the first 20 follows).

Folded in the **browse bulk multi-select** roadmap item: net-new `BulkFavoriteViewModel` (reusing `MangaLibraryAdder`) + `BulkSelectionToolbar` + `BulkFavoriteDialogs`, plus a `// RK` `selection` thread through the browse grids, the enhanced adult list, and the global-search rows; wired into source browse, global search, the follows screen, and the recommendations "See all" grid. Trims: one category choice per batch (no tri-state), no per-duplicate prompt.

**The two sync actions (Sync Follows to Library, Sync Library to MangaDex) moved to Phase 5**, their natural home. Manual import already works via the follows browse + multi-select (Select -> select-all -> add).

**Phase 5, settings hub + sync actions. SHIPPED 2026-07-06 (`b16b2cad6`), on-device verified.** A net-new `SettingsMangaDexScreen : SearchableSettings` (Reikai does not use `ConfigurableSource.getPreferenceScreen()`, 0 usages), wired like `SettingsEhScreen`: a `// RK` top-level entry in `SettingsMainScreen` gated on `isEnabled()` (a MangaDex source enabled), plus a `SettingsSearchScreen` entry. Rows:

- **Account** (`TextPreference` with `onClick`): a tappable "manage sign-in under Tracking" row that navigates to `SettingsTrackingScreen`. Login lives on the `MdList` tracker (Phase 3), not here; this row just orients the user so they do not get lost looking for it.
- **Preferred MangaDex language** (`ListPreference` over `MdUtil.getEnabledMangaDexs(...)` -> `preferredMangaDexId`).
- **Follow statuses to import** (`MultiSelectListPreference` -> `mangadexSyncToLibraryIndexes`, the 7 `FollowStatus` values), the native-DSL replacement for Komikku's custom `SyncMangaDexDialog`.
- **Sync Follows to Library** and **Sync Library to MangaDex** (`TextPreference` rows, gated on `mdList.isLoggedIn`), each enqueuing the worker.

Both actions run in one net-new `MangaDexSyncWorker : CoroutineWorker` (import/push via a target data param, patterned on `EHentaiUpdateWorker`; foreground progress via a new `Notifications.ID_MANGADEX_PROGRESS`), the dedicated-job path (decision §2), never `LibraryUpdateJob`.

- **Sync Follows to Library:** `fetchAllFollows()` filtered by the chosen `FollowStatus` set, then per item `NetworkToLocalManga` + `UpdateMangaFromRemote(fetchDetails, fetchChapters)` + favourite + `InsertFlatMetadata`. Re-typed from Komikku's `syncFollows`, which reuses the pre-fetched `SManga` via `awaitUpdateFromSource` (an efficiency Reikai mirrors if it drops in cleanly, else it accepts `UpdateMangaFromRemote`'s re-fetch).
- **Sync Library to MangaDex (full-track push, locked to Komikku's behaviour):** `GetLibraryManga.await()` filtered to `MANGADEX_IDS`, guard `mdList.isLoggedIn`, per item find-or-`createInitialTracker` the `MdList` track and flip `UNFOLLOWED` -> `READING` via `mdList.update` + `insertTrack`, so pushed entries become MdList-tracked and keep syncing (not a bare one-shot `updateFollowStatus`).
- **Advanced per-language prefs: not surfaced** (decision §3). The stock MangaDex extension already exposes data-saver / cover-quality etc. through its own Browse > Extensions settings, which the delegated source inherits, so a duplicate UI (and its source-side read wiring) is dropped. The two prefs (`preferredMangaDexId` + `mangadexSyncToLibraryIndexes`) live on the existing `ReikaiSourcePreferences` (Reikai's already-registered analog of Mihon's `SourcePreferences`), not a net-new class; under `reikai.**` so no new R8 keep.

**What shipped (refinements over the plan above):** the worker is `MangaDexSyncJob` (`exh/md`). The two sync rows are **always visible** (not gated on `mdList.isLoggedIn`): Mihon drops disabled preference rows entirely, so tapping one while signed out shows a "sign in under Tracking first" toast instead of the row vanishing. Row titles/summaries match Komikku's wording. The category icon is `MangaDexLogo` (`exh/md`), the MD mascot built as a tinted `ImageVector` from the brand vector's paths (matching Komikku), since `SettingsMainScreen` icons are static `ImageVector`s and cannot load a drawable resource. On-device (minified `preview`, real account): import brought in a follow with cover, metadata and 94 chapters; push flipped it plan-to-read -> reading (confirmed on the account).

**Phase 6, optional polish. Random button SHIPPED 2026-07-06 (`c6ad2ada7`), on-device verified; the rest evaluated and dropped this session.**

- **Browse "Random" button (shipped).** A `// RK` Random button on the MangaDex Browse filter sheet (left of the Follows button), gated on `getMainSource<MangaDex>()`. Net-new `RandomMangaSource` source-api interface (`fetchRandomMangaUrl`), implemented by `MangaDex` via `MangaHandler.fetchRandomMangaId()` over the already-ported `MangaDexService.randomManga()` (`/manga/random`). The fetch is async, so the click sets a one-shot `randomMangaTarget` on the browse state and a `LaunchedEffect` navigates to `BrowseSourceScreen(sourceId, "id:<uuid>")` (the stock extension's `id:` search hydrates it via delegation), avoiding a push from an async callback. Mirrors the P4 Follows-button port; no new R8 keep.
- **Similar-manga carousel: dropped.** It would only be one more provider in Reikai's existing related/recommendations carousel (the `SourceNative` lane), not a new UI. Its sole data source, `api.similarmanga.com` (the `similar-manga/similar` TF-IDF project), is live but **frozen at 2025-05-27 and unmaintained**, with inconsistent quality (weak, near-random matches for major titles like Chainsaw Man / One Piece). The tracker-based recs already cover popular titles better, and MangaDex's official `/relation` endpoint returns exact relations (doujinshi / colored), not discovery. Not worth a dependency on an abandoned API. (curl-verified live 2026-07-06)
- **External-aggregator page handlers (MangaPlus, Bilibili, Comikey, Azuki, MangaHot, Namicomi): dropped as redundant.** Reikai's `MangaDex` is a `DelegatedHttpSource` that delegates page fetching to the stock extension, which already handles these aggregators, so porting Komikku's ~490 fragile LOC (MangaPlus XOR-cipher interceptor, Bilibili RxJava + POST token) would duplicate what delegation gives for free.
- **Cover / data-saver / description niceties: dropped as low-value.** data-saver and cover-quality are already exposed by the stock extension's own settings (Browse > Extensions); first-volume-cover and alt-titles / final-chapter-in-description are minor cosmetics needing per-language pref-read wiring + i18n for little payoff.
...
A final path-tracing audit (`code-research`, four parallel explorers plus inline adversarial re-reading) compared the shipped port against `refs/komikku`'s current MangaDex source. Komikku's version currently has three live bugs, all confirmed **fixed in Reikai** (SAFE), verified both by tracing the code in both trees and on-device (Reikai works, Komikku does not).

All three share **one root cause**: Komikku drops the extension's `Tachiyomi` User-Agent on its own MangaDex API calls, and the API rejects a browser UA (400/401). Komikku builds `MangaDexService(client)` (no `headers`) and refreshes tokens on `MangaDexLoginHelper(network.client)`; Reikai threads the delegate `headers` through `MangaDexService(client, headers)` on every request and refreshes on the delegate `client` (extension UA). This is the same fix Phase 1 (`28ba52d7b`) and Phase 3 landed, applied consistently.

- **Details 400 (open a title):** Komikku's `viewManga` GET carries no headers -> 400. Reikai passes `headers`. SAFE.
- **Random button crash:** Komikku's `randomManga` GET carries no headers -> 400 HTML body -> `awaitSuccess().parseAs()` throws -> uncaught crash. Reikai passes `headers`, so the request succeeds. SAFE. (Residual: `MangaHandler.fetchRandomMangaId` is an unguarded `service.randomManga().data.id` in both trees; Reikai is safe only because its request succeeds. A try/catch would harden it, optional.)
- **Follows 401:** Komikku's token refresh runs on `network.client` (browser UA), fails silently, and leaves a stale bearer -> 401. Reikai refreshes on the delegate client with headers and routes follows through the interceptor-bearing `authClient`. SAFE.

Bonus: Reikai's `FollowsHandler` also fixes a Komikku follows-pagination skip (Reikai steps the offset by `followsPageLimit * (page - 1)`; Komikku stepped `mangaLimit * page`, dropping entries on page 2+). No other real findings: `ApiMangaParser` re-throws (no silent-fail), `MdUtil.createMangaEntry` uses the correct receiver-property `apply {}` (no shadowing), and the agent-flagged "lost first-volume cover" is a false positive (Komikku gates it on a default-off pref, so both trees read the same coverArt relationship). A future Komikku sync must not "fix back" to the headerless `MangaDexService(client)` / `network.client`-refresh pattern.
...
2. **Follows-sync worker shape. RESOLVED (2026-07-06): dedicated job.** One `MangaDexSyncWorker : CoroutineWorker` with an import/push target param (patterned on `EHentaiUpdateWorker`), enqueued via `context.workManager`, not a `LibraryUpdateJob` target. Push depth also locked to Komikku's full-track behaviour (find-or-`createInitialTracker` + `mdList.update` + `insertTrack`), not a bare `updateFollowStatus`.
3. **Advanced prefs UI. RESOLVED (2026-07-06): not surfaced.** The stock MangaDex extension already exposes data-saver / cover-quality through its own extension settings (Browse > Extensions > MangaDex), which the delegated source inherits, so Reikai does not duplicate them or their source-side read wiring. `MangaDexPreferences` holds only `preferredMangaDexId` + `mangadexSyncToLibraryIndexes`.
...
6. **Preferred source and description switches. RESOLVED (owner, 2026-09-24 audit ruling).** `MdUtil.getEnabledMangaDex` reads `preferredMangaDexId` itself and falls back to the first enabled source, and `MdList` resolves the source on every call instead of caching it, so a changed preference reaches the tracker, sync and login alike without a restart. The enhanced source honours the extension's own "Alternative titles in description" and "Final chapter in description" switches, read from the extension's `source_<id>` preferences under its `<pref>_<dexLang>` keys with the extension's defaults (off and on), and appends the same blocks the extension would. This supersedes the Phase 1 and Phase 5 notes above that dropped the description branches.
