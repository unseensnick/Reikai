...
Folded in the **browse bulk multi-select** roadmap item: net-new `BulkFavoriteScreenModel` (reusing `MangaLibraryAdder`) + `BulkSelectionToolbar` + `BulkFavoriteDialogs`, plus a `// RK` `selection` thread through the browse grids, the enhanced adult list, and the global-search rows; wired into source browse, global search, the follows screen, and the recommendations "See all" grid. Trims: one category choice per batch (no tri-state), no per-duplicate prompt.
