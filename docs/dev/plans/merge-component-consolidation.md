...
- **DI.** `ReikaiBindings` provides two singletons (`MangaMergeManager`, `NovelMergeManager`). This becomes two instances of the one class, one per content type, distinguished so injectors resolve the right one. Options: two typealias-qualified factories, a Metro qualifier per type, or thin `MangaMergeManager` / `NovelMergeManager` subclasses that fix `contentType` (subclasses keep the ~24 injection sites resolving by type with zero DI churn, at the cost of two tiny classes). Decide in the plan; the subclass route is the smallest blast radius.
...
- DI: `mihon/app/di/ReikaiBindings` (the two manager providers).
...
1. **DI shape for the unified manager**: two `EntryMergeManager` instances via Metro qualifiers, or two thin `Manga`/`Novel` subclasses that fix `contentType` (smallest blast radius, keeps ~24 injection sites resolving by type). Recommend the subclass route unless a qualifier reads cleaner.
