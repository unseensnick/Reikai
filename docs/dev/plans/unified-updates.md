...
**The All / Manga / Novels chip.** The chip's selection is a sticky preference (`ReikaiSourcePreferences.updatesContentType`), exposed by `NovelUpdatesViewModel.contentType` as a `StateFlow` and rendered by the shared `ContentTypeFilterChips` composable. The tab (`UpdatesTab.Content()`) reads it and passes the current `ContentType` into the shell. All three chips render through the same `ReikaiUpdatesScreen`; there is no separate manga screen anymore (`UpdatesTab.kt`).

**Two models behind one shell.** `UpdatesTab` builds both Mihon's stock `UpdatesViewModel` (manga) and Reikai's `NovelUpdatesViewModel` (novels) and hands both to `ReikaiUpdatesScreen`, which reads each one's `state` (`ReikaiUpdatesScreen.kt`). Both manga and novel rows render through the shared `reikai/presentation/updates/EntryUpdatesRow.kt` composable, each fed by a per-type mapper (so the two row layouts cannot drift). What stays per-type is the model behind the row: every manga action still calls a public method on the untouched Mihon model (`updateLibrary`, `markUpdatesRead`, `bookmarkUpdates`, `downloadChapters`, `showConfirmDeleteChapters`, `toggleSelection`, `invertSelection`, `showFilterDialog`, and so on). Because the manga path drives off Mihon's model verbatim, the Manga chip behaves exactly like stock Mihon and stays easy to port.
...
- `eu/kanade/tachiyomi/ui/updates/UpdatesTab.kt`: the Voyager `Tab`, now a thin host over the shared recents screen (`RecentsTabBody`; badge reset, bottom nav, reselect). Row taps open `ReaderActivity` for either content type through each recents adapter's `open`, and covers open `MangaScreen` / `NovelScreen`. Reikai additions fenced `// RK`.
- `reikai/presentation/recents/RecentsScreen.kt`: the shared recents screen Updates renders through since the recents takeover (it replaced the consolidated `ReikaiUpdatesScreen`): toolbar, combined selection action bar, pull-to-refresh. The collapsed-group and child rows live in `RecentsRows.kt`.
- `reikai/presentation/updates/NovelUpdatesViewModel.kt`: the novel side: recent-updates feed, shared filters, by-category filter, merge-aware series keys, selection, and chapter actions. Defines `NovelUpdatesItem`.
