...
Novel plugins now come back with the restore (`493252466`). A backup carries the plugin URLs, through the preference backup, but never their scripts, and the lazy re-download only covered the process the restore ran in: the in-memory restored-URL set was empty after any relaunch, so the plugins stayed listed while nothing could run, and the restore still reported zero errors. Two changes: a load that finds no stored script fetches it from the URL it was installed from (trust is unchanged, since revalidation drops any URL no added repo vouches for before a load happens), and `NovelPluginRestorer` reloads them inside the restore, bounded at 120s, naming each one that did not come back in the same log manga extensions use. Verified on a minified nightly with its data cleared: all four plugins returned with nothing installed by hand.
