...
**Proto classes (proto = Protocol Buffers, the compact binary format backups serialize to).** Each novel concept gets a backup model that mirrors its Mihon manga counterpart: `BackupNovel`, `BackupNovelChapter`, `BackupNovelTracking`, `BackupNovelHistory`, `BackupNovelCategory`, and (in `BackupNovelMerge.kt`) `BackupNovelMergeGroup`, a group serialized as a list of `BackupNovelSourceRef` (`{url, source}`) refs. Every field carries a `@ProtoNumber`. The novel classes are grafted onto the root `Backup` object in the 700 number range, fenced with `// RK`, so they sit clear of Mihon's own fields (1-106) and Komikku's (600/610) and won't collide if upstream or Komikku patches are pulled in later. The root fields are `backupNovels` (700), `backupNovelCategories` (701), `backupNovelMerges` (702), and `backupNovelUnmerges` (703). `backupNovelSources` (717) carries each novel source's name, the twin of Mihon's `backupSources` (101), so the pre-restore warning and the restore log can name a source that is not installed; a backup without it falls back to the source id.

**Creating the backup.** `NovelBackupCreator` builds the proto objects, and it and `MangaBackupCreator` are the two adapters of one driver (`reikai/data/backup/BackupEntryDriver.kt`), which decides which series are backed up and which parts each carries. It always adds merge-group members outside the library (written with favorite off), because the groups are always written and a restore resolves a member only against a row it restored (owner, 2026-09-24; `GroupMemberBackupConformanceTest`). `BackupCreator` streams both through one writer, gated by the same `BackupOptions` toggles as manga (no separate novel toggle, since novels are library content like any other). History rows come from a new query pair on `novel_history.sq`: one read for backup, one idempotent upsert for restore.

**Restoring.** `NovelRestorer` mirrors `MangaRestorer`: it does a match-or-insert by `url + source` (find an existing novel with that source-and-url pair, otherwise insert a fresh row), then re-links the child data by stable keys rather than by id, since ids are assigned fresh on insert: chapters by url, categories by name, tracks by tracker id, history by chapter url. Where a series is already on the device, the two restorers merge by one set of rules in `reikai/data/backup/RestoreMergeRules.kt`, pinned by `RestoreMergeConformanceTest`: the higher version's details win, a chapter keeps read and bookmark from either side and the further progress (a deliberate departure from Mihon, whose manga restore let any non-zero backup position win and so could rewind the device), and a bound track keeps the device's row, taking only the backup's remote link and the further chapter read (owner, 2026-09-24). `BackupRestorer` runs the novel restore as a self-contained sequential job (categories first, then each novel, then the merges) so it doesn't race the parallel manga and preference restore jobs. After the restore settles, the novel category-id preferences (the library / update filters and the default category) are remapped by category name to the freshly minted ids, so a saved filter follows its category instead of going stale; see [category-schema-unification.md](category-schema-unification.md).

**Merge/grouping surviving id reassignment.** Merge groups live in the DB (the `merge_group` + per-type member tables), not preferences, since the [merge system rebuild](merge-system-rebuild.md). Internal novel ids are reassigned on restore, so a group is serialized into the backup as stable `{url, source}` refs (`BackupNovelMergeGroup` / `BackupNovelSourceRef`, read from `MergeGroupRepository.getAllMemberships`). `NovelRestorer.restoreMerges` resolves each ref to its freshly restored id and hands the groups to `RestoreMergeGroups`, which both types share and which writes each through `MergeGroupRepository.materializeGroup` in one transaction: the backup decides the group of every entry it names, and local members it does not name keep their own group while two or more remain. The unmerge concept was retired in the rebuild, so unmerge refs are no longer restored (the `backupNovelUnmerges` proto field stays declared only so older backups still parse). A pre-rebuild backup's id-keyed merge preference keys are still skipped in the generic preference restore (`PreferenceRestorer`, a `// RK` skip) so their stale raw ids aren't written back.

Manga merges use the same `{url, source}` re-keying, since manga ids are reassigned on restore too. `BackupMangaMerge` holds `{url, source}` refs (root proto field 711; 712 is no longer written or read), `BackupCreator` serializes each manga group off `getAllMemberships`, and `MangaRestorer.restoreMerges` resolves the refs and hands them to the same `RestoreMergeGroups` (covered by `MangaMergeBackupRoundTripTest`). The manga restore also runs categories first: `BackupRestorer` awaits the category job before the manga and app-settings jobs, because `MangaRestorer.restoreCategories` maps each backup category to a live one by name, so the categories must already be in the database or the entry falls into the Default category.
...
- Extension apps: `BackupExtension` records the installed manga and novel extension apps (`ExtensionBackupCreator` writes them). On restore, `ExtensionRestorer` runs after the repos are restored, looks each one up in the now-populated available list by package name, and reinstalls via the standard `ExtensionManager` installer (respecting the user's installer mode). The installs are awaited on the restore's own scope, bounded per install, so they complete within the restore rather than lingering on the app-lifetime scope and racing the trust evaluation, which would otherwise leave the same extension showing as both trusted and untrusted. The restored repo's signing key auto-trusts the reinstalled extensions; the manually-trusted list (`SourcePreferences.trustedExtensions`) is app-state and is never backed up, by design. Every extension that does not come back is written to the restore log with its reason: repo missing (no match), install failed, install cancelled (the installer reported the prompt dismissed), or install timed out.
- Novel plugins: no new proto is needed. A novel plugin's install state (its canonical URL plus metadata) already rides the preference backup, so once preferences restore, `NovelPluginRestorer` reloads each plugin from its saved URL inside the restore, bounded, and names each one that did not come back; the lazy loader (`LnPluginInstaller.ensureLoaded`) still heals any it missed on the next novel-screen open.
...
Supporting:
- `data/src/main/sqldelight/tachiyomi/data/novel_history.sq` (backup read + idempotent restore upsert)
...
Novel-plugin re-download runs inside the restore through `NovelPluginRestorer.kt`, after the app-settings restore that writes the plugin and repo URLs; the lazy loader (`LnPluginInstaller`) still heals any plugin that did not come back.
...
Novel plugins now come back with the restore (`493252466`). A backup carries the plugin URLs, through the preference backup, but never their scripts, and the lazy re-download only covered the process the restore ran in: the in-memory restored-URL set was empty after any relaunch, so the plugins stayed listed while nothing could run, and the restore still reported zero errors. Two changes: a load that finds no stored script fetches it from the URL it was installed from (trust is unchanged, since revalidation drops any URL no added repo vouches for before a load happens), and `NovelPluginRestorer` reloads them inside the restore, bounded at 120s, naming each one that did not come back in the same log manga extensions use. Verified on a minified nightly with its data cleared: all four plugins returned with nothing installed by hand. The restore also names an added repo it could not reach (nothing loads until every repo vouches for the list) and each plugin revalidation dropped because no added repo lists it any more.

Deferred follow-up (round 2, "Restore-path onboarding"): the restore log now names both halves, but it is still a log. The follow-up turns it into a guided post-restore step that surfaces the unmatched extensions and plugins as an actionable list, grouped by repo, that walks the user through reinstalling them.
...
- **Novel plugins reload inside the restore, bounded.** Leaving them to the lazy loader alone let a restore report no errors while every novel source was unusable after a relaunch, so `NovelPluginRestorer` reloads them within the restore, capped so one down repo cannot stall it, and names the ones that did not come back. Manga extensions need an explicit `ExtensionRestorer` for a different reason: Mihon backs up only the repos, not the installed set.
