...
- Manga extensions: `BackupExtension` records the installed manga extensions (`ExtensionBackupCreator` writes them). On restore, `ExtensionRestorer` runs after the repos are restored, looks each one up in the now-populated available list by package name, and reinstalls via the standard `ExtensionManager` installer (respecting the user's installer mode, fired on the manager's own scope so the restore job doesn't block on downloads). Anything whose repo is missing can't be matched and is written to the restore log.
