...
Deferred follow-up (round 2, "Restore-path onboarding"): today, manga extensions a restore can't bring back (no repo offers them, or their install failed, was cancelled or timed out) only land in the restore log, and a novel plugin is not reinstalled by the restore at all: it re-downloads on the next novel screen, so a plugin that fails there never reaches the log. The follow-up turns that passive log into a guided post-restore step that surfaces the unmatched extensions/plugins as an actionable list, grouped by repo, that walks the user through reinstalling them. It builds directly on the installed-sources backup shipped here.
