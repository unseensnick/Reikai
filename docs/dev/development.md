...
- The **database** is not interchangeable with the old Yōkai-based builds: the schema differs, so a Mihon-based install cannot open a Yōkai-Reikai `tachiyomi.db`. Since the 0.3.2 rename a Yōkai install never shares a data folder with this one, so the two databases cannot meet ([legacy-yokai-import.md](plans/legacy-yokai-import.md) records the importer that bridged them until then). **Backup files are** interchangeable: the proto is shared and unknown fields are skipped, so a Yōkai `.tachibk` restores its library, categories, chapters, history, tracking, settings and custom info here. Custom info sits on each entry at the field numbers Komikku and Yōkai use, so it restores both ways; a Reikai 0.3.x backup's root custom-info lists are folded onto their entries at decode, and a 0.3.x build restoring a newer backup loses that info. Per-category sort does not carry over (Yōkai keeps it in category field 800, which Reikai does not read). Category-id settings such as the default category keep only the Default category (id 0 in both apps), since Yōkai writes no category id (field 3) for `PreferenceRestorer` to map the others by.
...
- **Borrowed features** (Komikku / Tsundoku / IReader / LNReader): per-feature, no frontier. Record: [feature-ports.md](feature-ports.md).
