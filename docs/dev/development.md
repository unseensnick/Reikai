...
- The **database** is not interchangeable with the old Yōkai-based builds: the schema differs, so a Mihon-based install cannot open a Yōkai-Reikai `tachiyomi.db`. Since the 0.3.2 rename a Yōkai install never shares a data folder with this one, so the two databases cannot meet ([legacy-yokai-import.md](plans/legacy-yokai-import.md) records the importer that bridged them until then). **Backup files are** interchangeable: the proto is shared and unknown fields are skipped, so a Yōkai `.tachibk` restores its library, categories, chapters, history, tracking, settings and custom info here. Per-category sort does not carry over (Yōkai keeps it in category field 800, which Reikai does not read), and neither do category-id settings such as the default category, since Yōkai writes no category id (field 3) for `PreferenceRestorer` to remap them by.
