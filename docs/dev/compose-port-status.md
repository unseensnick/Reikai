...
- **`LibraryScreen`** - the **manga tab is now clean**; one shell-level rule-3 spot and the novel tab remain. (Rule 8 resolved 2026-05-29 Tier 1; rules 3/5/9 on the manga tab resolved 2026-05-29 Tier 2 phases 2A/2B.)
  - Rule 3: **manga tab RESOLVED.** `MangaLibraryTabContent` no longer injects anything (`PreferencesHelper` / `UiPreferences` / `SourceManager` / etc. all moved into `MangaLibraryScreenModel`). One residual: `Content()` (the tab shell) still reads `libraryActiveTab` via `Injekt.get<UiPreferences>()` at [LibraryScreen.kt:67](../../app/src/main/java/yokai/presentation/library/LibraryScreen.kt:67). Left deliberately: the shell hosts both the manga and novel ScreenModels and has no ScreenModel of its own; revisit if it gains one.
  - Rule 5: **manga tab RESOLVED.** The ~21 display/badge/layout/category pref reads now come from `LibraryTabState`; the composable derives only `columns` locally (needs screen width). Writes route through screen-model methods.
  - Rule 8: **RESOLVED 2026-05-29.** The `MangaDetailsController` launches and `PreMigrationController.navigateToMigration(...)` carry `// transitional: legacy ...` bridge comments. Same fix on `NovelLibraryTabContent`'s `NovelDetailsController` launch.
  - Rule 9: **manga tab RESOLVED.** Search + the suspend filter (track / download I/O) run in `MangaLibraryScreenModel`, gated (`distinctUntilChanged` + a `debounce` on the raw-library flow to collapse download-cache renewal storms) so they don't re-run on cosmetic changes. The composable keeps only the cheap, pure category-visibility / collapse / single-category pass plus `canMerge` / `canUnmerge` / `selectionHasRemoteSources` derivations (rule-9-permitted).
  - **Novel tab still pending (2C):** the same rule 3/5/9 debt remains in [NovelLibraryTabContent.kt](../../app/src/main/java/yokai/presentation/library/NovelLibraryTabContent.kt) (inline Injekt, ~30 pref reads with the shared-vs-independent duality, a `produceState` filter). Port it the same way, adding `BasePreferences` + `UiPreferences` to `NovelLibraryScreenModel` and resolving the shared/independent pref selection reactively.
