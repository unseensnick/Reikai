...
class LanraragiSearchMetadata : RaisedSearchMetadata() {
    ...
    override fun createMangaInfo(manga: SManga): SManga {
        ...
        return manga.copy().also { m ->
            ...
            m.genre = genres
            m.status = status
            m.description = summary ?: manga.description
        }
    }
    ...
    override fun getExtraInfoPairs(context: Context): List<Pair<String, String>> {
        return with(context) {
            listOfNotNull(
                getItem(arcId) { stringResource(MR.strings.id) },
                getItem(pageCount) { stringResource(MR.strings.page_count) },
                getItem(filename) { stringResource(MR.strings.filename) },
                getItem(extension) { stringResource(MR.strings.file_extension) },
                getItem(baseUrl) { stringResource(MR.strings.base_url) },
            )
        }
    }
