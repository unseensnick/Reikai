...
- **Port the upstream sync backlog in one pass** `[M]` - 33 commits above the synced base, 15 of them real code. Runs after the ViewModel migration so the four that touch renamed files apply cleanly, and before the history and updates redo because its largest commit (java.time to kotlinx.datetime) lands on the date handling both surfaces use. [Ledger](docs/dev/upstream-sync.md).
- **Redo history and updates at the behaviour seam** `[M]` - both got only the UI-leaf seam, so their behaviour is still forked and each restates rules its siblings own. Scouted 2026-08-06: the category-filter item is not a local double-filter but an owed upstream change, and both feeds will follow upstream into SQL. Measured cost is low, history has had one upstream behaviour commit in twelve months and updates three. Browse was scouted out of this item: its two engines diverge in pagination and filter dispatch, so it got the add-to-library verb collapse instead. [Plan](docs/dev/plans/unified-updates.md).
