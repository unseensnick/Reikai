...
- **"All" library chip (interleave manga + novels)** `[M]` - add an All option to the library content-type chip that shows both types in one view, beside the current Manga and Novels tabs. Interleaving entries is a view-layer merge of the two library screen models' item lists (both already emit `LibraryItem`); the tabbed per-category All depends on category unification below. Mixed-selection routing and cross-type sort are the design surface.
...
- **LN-plugin parsing hardening (from tsundoku's JsSource)** `[S-M]` - port the robustness helpers tsundoku's `JsSource` has that Reikai's LN-plugin host may lack: HTML-entity and control-char cleanup for SAF/XML safety, paged-chapter aggregation, next-page probing, relative-date parsing, and the LNReader filter -> `FilterList` conversion. A function-by-function diff of `JsSource` against `LnPluginSource`.
- **LN-plugin host compatibility (from tsundoku's JS runtime)** `[M]` - close the "fails silently" gaps in Reikai's LN-plugin host, each a small change to `headless.js` / `LnHostBridge`: add `Response.arrayBuffer()` + `Buffer` (plugins reading binary bodies currently throw), bridge in-app WebView cookies + the Laravel `X-XSRF-TOKEN` into plugin `fetch` (unlocks Inertia/Laravel sources), honor real `setTimeout` / `setInterval` delays (so politeness sleeps aren't collapsed to zero and trip rate limits), auto-heal truncated plugin downloads, and add a minimal `Blob` + broader `Response.headers`. Reikai is otherwise ahead of tsundoku here (real cheerio/htmlparser2/dayjs, `Intl`, typed storage, the restore trust-gate), so do not regress those.
