...
- **Content layer architecture (manga/novel unification, deep seam)** `[XL]` - one Reikai-owned shared behavior + UI layer over a neutral `Entry` vocabulary, with thin adapters: the manga `ScreenModel` stays live and synced, the novel one dissolves. Extends the shipped Entry* UI-leaf seam down into the ScreenModel behavior (the two details models are ~70-75% duplicate today), swaps the keep-inert dead-file policy for delete-and-manifest, and folds in the reader migration as the final phase. One surface at a time, verified: library, then browse, migrate/global-search, then reader, following the details surface plan as the pattern ([details surface](docs/dev/plans/content-layer-details-surface.md), [library surface](docs/dev/plans/content-layer-library-surface.md)). [Plan](docs/dev/plans/content-layer-architecture.md).

## Next

- **Extract the merge read/observe wiring into a shared host** `[M]` - collapse the per-ScreenModel group read/observe wiring (the related-ids state, the membership observer, the source-chip builder, the selected-source mirror) into a shared `EntryMergeGroupHost` both details models compose, mirroring `EntryMergeActionHost`, so a merge read behavior is written once. Delicate: it rewires the live manga model's reactive chapter/metadata paths and the novel model's sibling-source map (ranking + chapter routing + reader), so it needs on-device verification. Deep-scouted. [Plan](docs/dev/plans/merge-component-consolidation.md).
...
- **"All" library chip (interleave manga + novels)** `[M]` - add an All option to the library content-type chip that shows both types in one view, beside Manga and Novels. Now delivered by the **content-layer library phase**: once the library surface migrates onto the neutral `Entry` list and `EntryId` dispatch, All is just the unfiltered view of the shared library. The cross-type sort (one comparator over the merged list) and per-item action dispatch fall out of the shared behavior, replacing today's negative-id-sign routing and the whole-screen `isNovels` switch. Fold it into that phase rather than building it now on the disguise it would then retire. Co-requisite: the category-schema unification below (the shared category axis). See [content-layer-architecture.md](docs/dev/plans/content-layer-architecture.md).
- **Unify the category schema (content_type discriminator)** `[M-L]` - retire the forked `novel_categories` table into the shared `categories` table with a `content_type` column (manga / novel / universal type-0), tsundoku's model adapted to Reikai's separate entry tables (the two join tables stay), which fits the content-layer decision to keep the entry tables separate. The co-requisite **data half** of the "All" library chip above: the content-layer library phase gives All the merged list and dispatch, this gives it the shared category axis, and it also drops a parallel repo/model stack. Runs alongside the content layer, not inside it (it is a schema change, not behavior). Unlike the code-only `EntryId` refactor, this is a real migration and must preserve the novel-order encoding, so it touches schema and backups.
...
- **Manga library re-reads the metadata tables on every download tick** `[S]` - `LibraryScreenModel`'s favorites flow recomputes the EXH tag + alt-title reads inside the combine that also re-emits on download-cache changes, so an adult-metadata library re-scans both tables per tick during downloads (the double download-count walk was already fixed). Restructure the two reads into their own upstream flow. Revive if download-tick jank shows; riskier because it touches Mihon's core favorites flow.
