...
- **Close out the fourth migrate/merge audit** `[M]` - the round ran and most of it landed; what remains is the group-aggregation fix (a merge group can hold entries no longer in the library, owner-ruled preserve-the-group-and-fix-the-reads, blocked on the tracker hand-out decision), the merged-unread badge SQL that over-reports for such a group, the config screen's off-thread preference write, and two test gaps. Items and detail in the local round-4 audit note; standing rules in [content-layer-migrate-surface.md](docs/dev/plans/content-layer-migrate-surface.md).
- **Trim the migrate flow's comments onto the plan doc** `[S]` - roughly 1000 comment lines across three flow files against upstream's 2 in its whole migration tree, most of it narrating past defects the plan doc already records. Keep the invariant sentence, move the story; several encode invariants the audits proved load-bearing, so it is not a blanket cut.
- **Delete the `EntryMigrateController` trio** `[S]` - a `@Stable` box around one nullable `MutableState` plus a nine-line forwarder, bypassed by five of its nine call sites already (`HistoryTab` uses both shapes). Two APIs for one job is what the audits keep finding defects in.
