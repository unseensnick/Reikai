...
- **Adult-source browse parity** `[M]` (investigated 2026-06-30; port not yet started): the built-in EH browse loads only its first page (then "No results found"), can crash the page on an odd row, and shows bare thumbnail + title rows instead of the rich per-row info (rating, category, page count, language, uploader, date). Root cause: the adult subsystem is a post-rebase Komikku port, and its EH paging + metadata layer was never fully ported (it was collapsed into Mihon's generic paging): the `MetadataMangasPage` cursor carrier, the `EHentaiPagingSource`, and the metadata-rich browse row are all absent, so pagination falls back to `page + 1` while the site's `next=` is a gallery-id cursor (page 2 becomes `next=2` -> empty), and the parsed metadata is discarded at the return. Not a parser bug. Two-piece port: (A) the cursor fix + metadata carrier (small, element type unchanged), then (B) the rich rendering (the `Pair<Manga, metadata>` element-type change + `combineMetadata` DB-join + EH list row). See [adult-browse-parity.md](docs/dev/plans/adult-browse-parity.md). Supersedes the reverted skip-un-parseable-rows band-aid (it hid the crash but left pagination dead after page 1). Reported in unseensnick/Reikai#20.
...
- **Rich EXH metadata rendering (details surfaces)** `[M]`: port `SourceTagsUtil` plus the two manga-details surfaces that depend on it: namespaced tappable tag chips, and a per-source gallery info block above the description. Presentation only; the `*DescriptionAdapter` composables are live in Komikku, not dead. (The third surface, the EH-specific browse list row, is now folded into the **Now** "Adult-source browse parity" initiative, piece B.)
...
- Phase 2: built-in E-Hentai / ExHentai source (anonymous browse + read, full gallery filters, gallery-version chapters); Settings → Advanced "Enable adult sources" toggle; ExHentai WebView login; E-Hentai settings screen + server-profile sync (uconfig) (`41e85aac8`, `fc73b2adc`, `29d05b931`, `497b8d9f1`, `1f9ec4cab`, `692c98200`). **Known parity gap (found 2026-06-30):** browse paginates only the first page and renders bare rows; this part of the Komikku port (the paging + metadata-carrier layer) was never finished. Tracked under **Now** -> "Adult-source browse parity" ([adult-browse-parity.md](docs/dev/plans/adult-browse-parity.md)).
