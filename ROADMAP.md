...
- **"All" library chip, via an All-first library takeover** `[L]` - build All as the real library view and make the Manga and Novels chips filters over that one assembled list, so a spanning category shows once and holds both types. Remaining: assembly in the shared layer, the chip as a predicate, counts and empty categories, then the verb takeover after which `LibraryScreenModel` is deleted and manifested. [Plan](docs/dev/plans/library-all-chip.md).
