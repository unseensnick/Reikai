...
- **"All" library chip, via an All-first library takeover** `[L]` - show manga and novels in one view, the content-type chips becoming filters over a single assembled list (a spanning category shows once and holds both types); the shared layer takes over list assembly and the action verbs, after which `LibraryScreenModel` is deleted and manifested. Remaining phases: collapse the two library settings sheets into one, single-value the shared state, All-first assembly, verb takeover, parity closeout. [Plan](docs/dev/plans/content-layer-library-surface.md).
