...
- **Audit the migration and merge systems whole, a fourth time** `[M]` - three whole-system rounds have run (8 High + 14 Medium, then 3 High + 6 Medium, then 1 High + 5 Medium), and every round has found defects inside the previous round's fixes, including two of its own each time. It gets another pass at the SAME whole-system scope, not scoped to the last diff. Two constraints carry over: findings limited to situations a real user can reach (no concurrent-session or millisecond-interleaving defects; anything timing-dependent needs the gesture sequence that produces it), and the plan grounded for both sides before any edit, enumerating the real family per fix rather than assuming it is manga versus novel. Hold the standing rules in [content-layer-migrate-surface.md](docs/dev/plans/content-layer-migrate-surface.md) and plan structurally rather than patch per finding.
