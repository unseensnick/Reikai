...
**Unified content UI** on `feat/unified-content-ui` (unmerged): the History + Updates rows, the full-cover dialog, and the **details screen** (shared action row, info header, phone + tablet shell, toolbar, manga hide/unhide-chapters) are collapsed onto shared `Entry*` components. **P6 shipped** the shared edit-info editor + a non-destructive custom-info overlay for BOTH manga and novels: edits show across details + library + updates + history (display-only, so search/sort/grouping/merge use the source values), novels moved off their destructive in-row model onto a `custom_novel_info` overlay, the editor is themed from the cover, and its **Fill-from-tracker** button autofills title/author/artist/cover/description + genres from a bound tracker (8 trackers; a picker when several are bound). Verified device + minified. Detail: [the plan](docs/dev/plans/unified-content-ui.md); goal + rulings: Later -> UI & design. This, plus the 2026-07-09 audit phases below, completes **round 1**; the active forward program is now **round 2** (fix the remaining manga/novel drifts a 2026-07-13 deep audit found, then collapse browse / global search / migration / library settings onto the `Entry*` seam): [plan](docs/dev/plans/content-parity-drift-and-collapse.md).

## Next

After P6, a full-surface parity audit (2026-07-09) is closing the remaining manga/novel gaps in phases. Shipped on the branch: the silent-bug sweep, the library tracker dimensions (novel filter / sort / group by tracker + a manga download-count sort), novel duplicate-detection + browse bulk multi-select, the **reader-chrome unification** (shared top/bottom bars + rail + configurable novel bottom-bar buttons), and the **settings content-type reorg** (manga/novel options grouped and labeled per type across Reader / Downloads / Library / Advanced, with a shared `contentTypedCategory` helper and group-aware settings search). **Reader 7b shipped too:** volume-key navigation (invert option + a screen-fraction scroll-amount slider, the slider also extended to long-strip manga) and an always-on reading percentage (LNReader-style footer, live per scroll frame). The **reader skip-hidden fix** landed too: both readers now skip hidden chapters on next/previous, and the novel reader filters its own list instead of trusting the caller (fixing a leak where history / updates / library-resume walked into hidden chapters). The **download-queue unification** shipped too: manga and novels now share one per-series card queue (flat drag + to-top/to-bottom reorder, cancel-series, one combined Pause/Resume FAB, the All view stacking both types), the More badge counts novel downloads, and novels gained download **pause/resume** (Phase 11), landing alongside a Phase 9 cleanup batch. The **novel download re-key** shipped too (Phase 10): novel downloads now use stable, readable folders (source / title / chapter) with disk-scan recognition, so they survive reinstall / restore / storage-move, plus a one-time migration for existing downloads. The manga and novel libraries now also keep independent scroll positions (each remembers its own, instead of sharing one offset), and the novel chapter list reached behaviour parity (missing-chapter indicators, plus a fix for the reversed swipe direction). **Round 2 is underway:** the drift fixes (Phase 1) and the browse-grid-cell, source-options-dialog, and global-search collapses (2a, 2b, 3) are shipped on the branch; the 2c browse-shell collapse is parked. Round 2 collapses shipped through 4c (notes screen 4a, library Display tab 4b, shared category-diff + novel download queued-exclusion fix 4c); migration (5) is resolved with no new collapse (most was already shared, the config-scaffold collapse was assessed and declined, one search-tuning gap parked above). Left in round 2: the `[S]` novel quick wins; the track-info stack (6) is deferred. Then the `[S]` quick wins, then the download-subsystem unification (Road B, under Later -> Novels). The reader's tsundoku track (seamless transitions Option 1, then the Option 3 migration) is queued under Later -> Reader, *after* the parity phases, not ahead of them. Phase-by-phase status lives in `Handoff.md`. [Plan](docs/dev/plans/content-parity-drift-and-collapse.md).
...
(The round-2 drift fixes are shipped on the branch: merged-novel read/bookmark propagation, browse toolbar filter-reset + Filter-chip highlight, global-search empty-source sink + progress bar, first-download add-to-library prompt, migration blank-research guard. [Plan](docs/dev/plans/content-parity-drift-and-collapse.md).)

Larger:
- **Collapse the remaining manga/novel twin surfaces (round 2)** `[M-L]` - browse grid cells, the source options dialog, and novel global search are already collapsed onto the shared `Entry*` seam (shipped on the branch); the browse-shell/toolbar collapse (2c) is parked (irreconcilable paging + already-shared toolbars). Left to collapse: the notes screen (4a), the library Display tab + shared selection/category/download interactors (4b/4c), and migration config (5). The track-info Screen/Model stack (6) stays deferred. [Plan](docs/dev/plans/content-parity-drift-and-collapse.md).
- **Unify the download subsystem across manga and novels** `[L]` - collapse the parallel novel download cache/provider (added by the novel download re-key) into one shared disk-scan layer serving both content types, keyed on shared primitives, so the two can't drift, matching Tsundoku's single-subsystem model. Follows the novel download re-key, which deliberately mirrors the manga path scheme and cache shape so this becomes a code merge, not a data migration. Touches Mihon's shipped download files (`// RK`).

Opportunistic polish: Browse (Latest shortcut, Last-used, hide-in-library, per-row language, genre-tap-search); Tracking (start-date backfill, create-private-at-bind, friendlier Fill-from-tracker errors: no-entry-found on a 404 + null-message fallback); Updates/history (fast-scroll animation); Details (long-press-copy WebView URL, per-source scanlator filter for merged novels, per-category novel display settings `[M]`, novel tag-tap global search, novel long-press-favorite category shortcut).
