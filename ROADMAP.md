...
- **Content layer architecture (manga/novel unification, deep seam)** `[XL]` - one Reikai-owned shared behavior + UI layer over a neutral `Entry` vocabulary with thin per-type adapters, extending the shipped Entry* UI-leaf seam down into ScreenModel behavior. Next is the **library takeover** (the library rebuilt All-first, the Manga / Novel chips becoming filters over one assembled list), then browse, migrate/global-search, and finally the reader migration. [Plan](docs/dev/plans/content-layer-architecture.md).

## Next

- **Level novel library search up to the query AST** `[M]` - manga search runs on Mihon's AST (field terms like `title:`/`author:`/`genre:`, comparisons like `unread>5`) while novels still use the older plain-text matcher, so the two libraries answer the same typed query differently; the gap was taken deliberately when the AST was ported, to keep that sync faithful. Ships with the "All" chip (a mixed list needs one grammar), gating the fields novels cannot support (no artist, String source ids).
...
- **"All" library chip, via an All-first library takeover** `[L]` - show manga and novels in one view, the content-type chips becoming filters over a single assembled list (a spanning category shows once and holds both types); the shared layer takes over list assembly and the action verbs, after which `LibraryScreenModel` is deleted and manifested. Phased: close the behaviour seam, collapse the two library settings sheets into one, single-value the shared state, All-first assembly, verb takeover, parity closeout. [Plan](docs/dev/plans/content-layer-library-surface.md).
- **Hide empty categories in the manga library** `[S]` - the novel library drops a category with no entries from the grid and the manga library keeps it, so one category that spans both renders differently on each side; owner ruled level manga up. The change-category picker already reads the full category list, so hiding a category cannot make it unreachable.
