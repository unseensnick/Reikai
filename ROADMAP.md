...
- **Fix the remaining manga/novel drifts (round 2)** `[S-M]` - close the silent behavioural gaps a 2026-07-13 deep audit found in the still-twin surfaces: merged-novel read/bookmark cross-source propagation, the browse toolbar (filter carry-over on a Popular/Latest switch, Filter-chip highlight, Latest capability-guard), novel global-search empty-source sink plus a searching indicator, the first-download add-to-library prompt, and a migration blank-research guard. [Plan](docs/dev/plans/content-parity-drift-and-collapse.md).

Larger:
- **Collapse the remaining manga/novel twin surfaces (round 2)** `[L]` - move browse (grid cells plus chrome), novel global search, the library Display tab plus shared selection/category/download interactors, the notes screen, and migration config onto the shared `Entry*` seam so they cannot drift again; the track-info Screen/Model stack is deferred. [Plan](docs/dev/plans/content-parity-drift-and-collapse.md).
