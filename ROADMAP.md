...
- **Collapse the parallel merge components onto the Entry* seam** `[M]` - the last fork the merge-system rebuild left: the two near-twin merge managers become one, and the per-ScreenModel group read/observe wiring extracts into a shared host both models compose (mirroring `EntryMergeActionHost`), so a merge behavior is written once. Deep-scouted. [Plan](docs/dev/plans/merge-component-consolidation.md).
