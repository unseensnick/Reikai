...
- **Adult-source browse parity** `[M]` (investigated 2026-06-30; port not yet started): the built-in EH browse loads only its first page (then "No results found"), can crash the page on an odd row, and shows bare thumbnail + title rows instead of the rich per-row info (rating, category, page count, language, uploader, date). Root cause is the Mihon-rebase port collapsing Komikku's EH paging + metadata layer into Mihon's generic one: the `MetadataMangasPage` cursor carrier, the `EHentaiPagingSource`, and the metadata-rich browse row were all dropped, so pagination falls back to `page + 1` while the site's `next=` is a gallery-id cursor (page 2 becomes `next=2` -> empty), and the parsed metadata is discarded at the return. Not a parser bug. Two-piece port: (A) pagination + metadata carrier, then (B) rich browse rendering. See [adult-browse-parity.md](docs/dev/plans/adult-browse-parity.md). Supersedes the reverted skip-un-parseable-rows band-aid (it hid the crash but left pagination dead after page 1). Reported in unseensnick/Reikai#20.
