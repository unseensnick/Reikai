...
- **Content layer architecture (manga/novel unification, deep seam)** `[XL]` - one Reikai-owned shared behavior + UI layer over a neutral `Entry` vocabulary with thin per-type adapters, extending the shipped Entry* UI-leaf seam down into ScreenModel behavior. The taken-over surfaces are closed out; remaining: the recents takeover and the download unification below, then the reader migration. Global search stays excluded. [Plan](docs/dev/plans/content-layer-architecture.md).

## Next

- **The recents surface: history, updates and a combined feed** `[L]` - one engine behind both feed screens plus the combined title-centric feed requested in `unseensnick/Reikai#57`, behind a Settings preference that collapses the two tabs into a single Recents tab with four modes inside it (off keeps today's two tabs, rendering the same engine). Left to build: the engine flow over the providers, the four render policies, merge collapse, one shared search and selection, the cutover, the affordance harvest and the tab set. [Plan](docs/dev/plans/content-layer-recents-surface.md).
