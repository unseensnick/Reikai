...
- **Auto Webtoon Mode (smart reading-mode detection)** `[S]` - when a series has no manual reading-mode set, auto-pick Webtoon for long-strip content (manhwa / manhua / webtoon) from its genre tags and source name; everything else stays on the global default, and a manual per-series choice always wins. Metadata-driven classification (genre + source-name tokens, never image / aspect-ratio detection), originally TachiyomiSY's and taken via Komikku with its token lists verbatim (the `manga` and `comic` branches are deliberate false-positive suppressors, not dead weight). Diverges from Komikku on two points: default off (it ships on) and a fresh preference key. Also takes its distinct "reading webtoon style" toast, so the user is told why the mode changed. One Settings -> Reader toggle; manga-only, since webtoon-vs-paged is an image-layout distinction with no analog in reflowed novel text. New `MangaType` heuristic in the existing `exh/util/` (already R8-kept) + a `// RK` branch in `getMangaReadingMode()`, keeping its `resolveDefault` guard so the toolbar indicator still reports the user's own choice. From [discussion 40](https://github.com/unseensnick/Reikai/discussions/40).
...
- **Prune the auto-webtoon source-name lists for liveness** `[S]` - the classifier takes Komikku's source-name token lists verbatim, and some entries name long-dead sources. A liveness pass would trim them. Parked: an unmatched token is inert (no installed source, so no match), costing a few string comparisons rather than wrong behavior. Revive if the lists get unwieldy or a stale token turns out to cause a false positive.
